...
-- user_id / phone / code_hash are added WITHOUT DEFAULT so they cannot
-- silently receive '' and violate FK or uniqueness constraints.
-- They are added as nullable here; NOT NULL is enforced after cleanup below.
...
-- Drop any '' default that a previous partial migration may have left behind
ALTER TABLE phone_otps ALTER COLUMN user_id   DROP DEFAULT;
ALTER TABLE phone_otps ALTER COLUMN phone     DROP DEFAULT;
ALTER TABLE phone_otps ALTER COLUMN code_hash DROP DEFAULT;

-- Remove orphaned / empty rows before enforcing NOT NULL and FK
DELETE FROM phone_otps WHERE user_id IS NULL OR user_id = '';
DELETE FROM phone_otps WHERE phone    IS NULL OR phone    = '';
DELETE FROM phone_otps WHERE code_hash IS NULL OR code_hash = '';

-- Enforce NOT NULL now that bad rows are gone
ALTER TABLE phone_otps ALTER COLUMN user_id   SET NOT NULL;
ALTER TABLE phone_otps ALTER COLUMN phone     SET NOT NULL;
ALTER TABLE phone_otps ALTER COLUMN code_hash SET NOT NULL;
...
DELETE FROM notifications WHERE title   IS NULL OR title   = '';
DELETE FROM notifications WHERE body    IS NULL OR body    = '';
...
ALTER TABLE mpesa_transactions ALTER COLUMN user_id             DROP DEFAULT;
ALTER TABLE mpesa_transactions ALTER COLUMN merchant_request_id DROP DEFAULT;
ALTER TABLE mpesa_transactions ALTER COLUMN checkout_request_id DROP DEFAULT;

DELETE FROM mpesa_transactions WHERE user_id IS NULL OR user_id = '';

ALTER TABLE mpesa_transactions ALTER COLUMN user_id             SET NOT NULL;
ALTER TABLE mpesa_transactions ALTER COLUMN merchant_request_id SET NOT NULL;
ALTER TABLE mpesa_transactions ALTER COLUMN checkout_request_id SET NOT NULL;
...
ALTER TABLE subscription_logs ALTER COLUMN subscription_id DROP DEFAULT;
DELETE FROM subscription_logs WHERE subscription_id IS NULL OR subscription_id = '';
ALTER TABLE subscription_logs ALTER COLUMN subscription_id SET NOT NULL;
...
ALTER TABLE webhook_events ALTER COLUMN external_id  DROP DEFAULT;
ALTER TABLE webhook_events ALTER COLUMN event_type   DROP DEFAULT;
DELETE FROM webhook_events WHERE external_id IS NULL OR external_id = '';
DELETE FROM webhook_events WHERE event_type  IS NULL OR event_type  = '';
...
ALTER TABLE otp_codes ALTER COLUMN email     DROP DEFAULT;
ALTER TABLE otp_codes ALTER COLUMN code_hash DROP DEFAULT;
ALTER TABLE otp_codes ALTER COLUMN purpose   DROP DEFAULT;
DELETE FROM otp_codes WHERE email     IS NULL OR email     = '';
DELETE FROM otp_codes WHERE code_hash IS NULL OR code_hash = '';
DELETE FROM otp_codes WHERE purpose   IS NULL OR purpose   = '';
...
ALTER TABLE embeddings ALTER COLUMN user_id DROP DEFAULT;
ALTER TABLE embeddings ALTER COLUMN text    DROP DEFAULT;
DELETE FROM embeddings WHERE user_id IS NULL OR user_id = '';
ALTER TABLE embeddings ALTER COLUMN user_id SET NOT NULL;
ALTER TABLE embeddings ALTER COLUMN text    SET NOT NULL;
...
DELETE FROM user_memories WHERE user_id IS NULL OR user_id = '';
ALTER TABLE user_memories ALTER COLUMN user_id SET NOT NULL;
...
ALTER TABLE nutrition_logs ALTER COLUMN user_id DROP DEFAULT;
ALTER TABLE nutrition_logs ALTER COLUMN name   DROP DEFAULT;
DELETE FROM nutrition_logs WHERE user_id IS NULL OR user_id = '';
DELETE FROM nutrition_logs WHERE name    IS NULL OR name    = '';
ALTER TABLE nutrition_logs ALTER COLUMN user_id SET NOT NULL;
ALTER TABLE nutrition_logs ALTER COLUMN name   SET NOT NULL;

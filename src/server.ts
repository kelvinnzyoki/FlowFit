...
// e.g. CORS_ORIGIN=https://yourapp.github.io,https://aicoach-snowy.vercel.app
// Wildcards ('*') are intentionally forbidden here — credentials:true requires
// an explicit origin match (browser will reject '*' + credentials).
const RAW_ORIGINS = (process.env.CORS_ORIGIN || '').split(',').map(o => o.trim()).filter(Boolean);
const ALLOWED_ORIGINS: Set<string> = new Set(
  RAW_ORIGINS.length > 0
    ? RAW_ORIGINS
    : ['http://localhost:3000', 'http://localhost:5173'] // safe dev fallback
);
...
// ── Pass OPTIONS preflights through before rate limiting ───────────────────────
app.options('*', cors({
  origin: (origin, callback) => {
    if (!origin) return callback(null, true);
    if (ALLOWED_ORIGINS.has(origin)) return callback(null, true);
...
app.use(cookieParser()); // Must be before routes so req.cookies is populated (refresh token)
app.use(compression());

...
// ── Webhook routes — raw body BEFORE global express.json() ────────────────────
// Paystack HMAC-SHA512 signature is computed over the raw request body Buffer.
// Parsing to JSON first corrupts the comparison and causes all webhooks to fail.
app.use(
...
// M-Pesa callback also needs raw body for its own HMAC verification
app.use(
  '/api/webhooks/mpesa/callback',
  express.raw({ type: 'application/json' }),
  paystackWebhookRoutes,   // M-Pesa handler is in the same file (separate case in processEvent)
);
...
// ── Named route groups mounted explicitly ──────────────────────────────────────
// subscriptionRoutes was imported but never registered — every /subscriptions/*
// endpoint (checkout, verify, cancel, plans) was returning 404 silently.

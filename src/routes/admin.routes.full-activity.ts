...
router.get('/users/:id/activity', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const limit = Math.min(PAGE_SIZE_MAX, asInt(req.query.limit, 50));
    ...
    const [
      ...
      prisma.programEnrollment.findMany({
        ...
        orderBy: { createdAt: 'desc' },
        take: limit,
        include: { program: { select: { id: true, name: true, category: true, difficulty: true, durationWeeks: true } } },
      }),
      prisma.subscription.findMany({
        where: { userId },
        orderBy: { createdAt: 'desc' },
      ...
      (prisma as any).notification?.findMany
        ? (prisma as any).notification.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, take: limit })
        : Promise.resolve([]),
    ...
    const memoryData = memory?.data && typeof memory.data === 'object' ? (memory.data as any) : {};
    const coachHistory = Array.isArray(memoryData.coachHistory) ? memoryData.coachHistory : [];

    const activity = [
      {
        type: 'USER_CREATED',
        label: 'Account created',
        message: `${user.name || user.email} joined FlowFit`,
      ...
      ...(user.lastLogin ? [{
        type: 'LOGIN',
        label: 'Last login',
        message: `${user.email} last opened FlowFit`,
      ...
      ...workoutLogs.map((w: any) => ({
        ...
        message: `${w.exercise?.name || w.exerciseName || 'Workout'} · ${w.duration || 0} min · ${Math.round(Number(w.caloriesBurned || 0))} kcal`,
        createdAt: w.createdAt || w.date,
        meta: { workoutLogId: w.id, exerciseId: w.exerciseId },
      })),
      ...enrollments.map((e: any) => ({
        type: 'PROGRAM_ENROLLED',
        label: e.isActive ? 'Program active' : 'Program enrollment',
        message: `${e.program?.name || 'Program'} · ${e.progress || 0}% progress`,
        createdAt: e.updatedAt || e.createdAt,
        meta: { enrollmentId: e.id, programId: e.programId },
      })),
      ...subscriptions.map((s: any) => ({
        type: 'SUBSCRIPTION',
        label: `Subscription ${s.status}`,
        message: `${s.plan?.name || s.plan?.slug || 'Plan'} · ${s.interval || 'MONTHLY'}`,
        createdAt: s.updatedAt || s.createdAt,
        meta: { subscriptionId: s.id, planId: s.planId },
      })),
      ...feedback.map((f: any) => ({
        type: 'FEEDBACK',
        label: `Feedback ${f.status}`,
        message: `${f.type || 'Feedback'} · ${String(f.message || '').slice(0, 120)}`,
        createdAt: f.updatedAt || f.createdAt,
        meta: { feedbackId: f.id, pageUrl: f.pageUrl },
      })),
      ...notifications.map((n: any) => ({
        type: 'NOTIFICATION',
        label: n.readAt ? 'Notification read' : 'Notification sent',
        message: `${n.title || n.type || 'Notification'} · ${n.message || ''}`,
        createdAt: n.createdAt,
        meta: { notificationId: n.id, readAt: n.readAt },
      })),
      ...metrics.map((m: any) => ({
        type: 'METRICS',
        label: 'Body metrics updated',
        message: `Weight ${m.weight ?? '—'} kg · BMI ${m.bmi ?? '—'}`,
      ...
      ...achievements.map((a: any) => ({
        ...
        message: `${a.achievement?.name || 'Achievement'} · ${a.achievement?.points || 0} points`,
        createdAt: a.unlockedAt || a.createdAt,
        meta: { achievementId: a.achievementId },
      })),
      ...coachHistory.map((h: any) => ({
        type: 'AI_COACH',
        label: 'AI coach exchange',
        message: String(h.user || '').slice(0, 160),
        createdAt: h.createdAt || user.updatedAt,
        meta: { assistant: String(h.assistant || '').slice(0, 300) },
      })),
    ]

...
router.get("/subscription-health", adminGuard, async (_req, res) => {
  try {
    const base = { provider: PaymentProvider.PAYSTACK, status: { in: [SubscriptionStatus.ACTIVE, SubscriptionStatus.PAST_DUE, SubscriptionStatus.TRIALING] } };
...
// ── POST /api/v1/admin/fetch-email-token ──────────────────────────────────────
// Fetches email_token from Paystack API using a SUB_xxx code visible in the
// Paystack dashboard, then updates the matching DB row.
//
// Usage:
//   POST /api/v1/admin/fetch-email-token
//   Header: x-admin-secret: <ADMIN_SECRET>
//   Body:   { "subscriptionCode": "SUB_xxxxxxxxxxxxxxxxx" }
//
router.post("/fetch-email-token", adminGuard, async (req: Request, res: Response) => {
  const SECRET_KEY = process.env.PAYSTACK_SECRET_KEY;
  if (!SECRET_KEY) { res.status(500).json({ error: "PAYSTACK_SECRET_KEY not set." }); return; }

  const { subscriptionCode } = req.body;
  if (!subscriptionCode) {
    res.status(400).json({ error: "subscriptionCode required in body." });
    return;
  }
  ...
  try {
    ...
    const json = await r.json() as { status: boolean; data: any; message: string };

    if (!json.status) {
      res.status(400).json({ error: `Paystack: ${json.message}` });
      return;
    }

    const emailToken = json.data?.email_token as string | undefined;

    if (!emailToken) {
      res.status(404).json({
        error: "Paystack returned this subscription but email_token was empty.",
        raw:   json.data,
      });
      return;
    }
    ...
    if (updated.count === 0) {
      // subscriptionCode not yet in DB — return the token so you can SQL-update manually
      res.status(404).json({
        error:      "No DB row found with this subscription code.",
        emailToken,
        hint:       "Run this SQL to fix manually:",
        sql:        `UPDATE subscriptions SET "paystackSubscriptionCode" = '${subscriptionCode}', "paystackEmailToken" = '${emailToken}' WHERE "userId" = 'your-user-id';`,
        portalUrl:  `https://paystack.com/manage/subscriptions/${emailToken}`,
      });
      return;
    }

...
async function verifyOtp(email: string, code: string, purpose: string): Promise<boolean> {
  ...
  if (purpose === 'registration') {
    updates.push(prisma.user.updateMany({ where: { email }, data: { isEmailVerified: true } }));
  }
...
router.post('/register', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !normalisedEmail || !password || !otp) {
      res.status(400).json({
        success: false,
    ...
    if (existing) {
      res.status(409).json({ success: false, error: 'This email is already registered. Please login instead.' });
      return;
    }
    ...
    if (!record) {
      res.status(400).json({ success: false, error: 'Incorrect or expired verification code. Please request a new one.' });
      return;
    }

    const validOtp = await bcrypt.compare(String(otp), record.codeHash);
    if (!validOtp) {
      res.status(400).json({ success: false, error: 'Incorrect or expired verification code. Please try again.' });
      return;
    }
    ...
    const user = await prisma.$transaction(async (tx) => {
      ...
      await tx.otpCode.update({ where: { id: record.id }, data: { usedAt: new Date() } });
      return newUser;
    });
...
router.post('/login', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // Welcome notification is created during registration. Do not create it again
    // on first login, otherwise users see duplicate welcome notifications after
    // logging out and logging back in for the first time.
...
router.post('/refresh', async (req: Request, res: Response) => {
  ...
  if (!refreshToken) {
    // ff_refresh is absent but ff_session may still be set — clear every auth
    // cookie so TokenManager.hasSession() returns false on the next page load
    // and the frontend stops retrying the refresh in an infinite loop.
    clearAllCookies(res);
  ...
  try {
    ...
    // Token is expired or tampered — clear all cookies so ff_session is gone
    // and the frontend's hasSession() returns false, breaking the refresh loop.
    clearAllCookies(res);

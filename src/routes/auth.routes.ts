...
// ─── Cookie config ────────────────────────────────────────────────────────────
// refreshToken lives in an HTTP-only cookie — JS can never read or steal it.
// accessToken is returned in the JSON body so api.js stores it in localStorage
// and sends it as Authorization: Bearer on every request.
//
// sameSite:'none' + secure:true is required for cross-origin requests
// (GitHub Pages frontend → Vercel backend are different origins).
// sameSite:'lax' is used in development where both run on localhost.
...
const REFRESH_COOKIE_OPTIONS = {
  ...
  maxAge:   7 * 24 * 60 * 60 * 1000,     // 7 days in ms (matches REFRESH_EXPIRES)
  path:     '/',
};
...
router.post('/login', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { user: safeUser, accessToken },   // refreshToken is in the httpOnly cookie
    });
  } catch (error) {
...
router.post('/logout', async (req: Request, res: Response) => {
  try {
    // Accept refreshToken from HTTP-only cookie (primary) or body (back-compat)
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    // Read from HTTP-only cookie (primary) or body (back-compat)
    ...
    res.status(200).json({
      success: true,
      data: { accessToken: tokens.accessToken },   // new refreshToken is in the cookie
    });
  } catch {
    res.status(401).json({ success: false, error: 'Invalid or expired refresh token.' });
  }

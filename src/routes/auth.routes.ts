...
router.post('/login', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // Fire welcome notification on the very first login (lastLogin was null before
    // this update). This acts as a safety net for users whose registration-time
    // notification failed, ensuring every user sees a welcome message.
    if (!user.lastLogin) {
      notifyWelcome(user.id, user.name || 'Athlete').catch(err =>
        console.error('[auth] notifyWelcome (first login) failed:', err)
      );
    }

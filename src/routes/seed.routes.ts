...
// Seeds the achievements table with standard fitness milestones.
// Same secret-protection pattern as seed-plans.
// Safe to call multiple times — uses upsert on name so nothing is duplicated.
router.post('/seed-achievements', async (req: Request, res: Response) => {
  ...
  const ACHIEVEMENTS = [
    ...
    },
    {
      ...
      icon: '🔥',
      category: 'MILESTONE',
      requirement: { type: 'totalWorkouts', value: 5 },
      points: 25,
    },
    {
      name: 'Committed',
    ...
    // ── Calorie milestones ────────────────────────────────────────────────────
    {
      ...
      icon: '🌡️',
      category: 'CALORIES',
      requirement: { type: 'totalCalories', value: 1000 },
      points: 50,
    },
    {
      name: 'Inferno',
      ...
      icon: '🔥',
      category: 'CALORIES',
      requirement: { type: 'totalCalories', value: 5000 },
      points: 150,
    },
    {
      name: 'Furnace',
      ...
      icon: '♨️',
      category: 'CALORIES',
      requirement: { type: 'totalCalories', value: 10000 },
      points: 300,
    },
    // ── Duration milestones ───────────────────────────────────────────────────
    {
      ...
      icon: '⏱️',
      category: 'DURATION',
      requirement: { type: 'totalDuration', value: 60 },
      points: 20,
    },
    {
      name: 'Hour Power',
      description: 'Log 300 minutes (5 hours) of total workout time.',
      icon: '⏰',
      category: 'DURATION',
      requirement: { type: 'totalDuration', value: 300 },
      points: 80,
    },
    {
      name: 'Time Lord',
      description: 'Log 1,000 minutes of total workout time.',
      icon: '🕰️',
      category: 'DURATION',
      requirement: { type: 'totalDuration', value: 1000 },
      points: 250,
    },
  ];
  ...
  try {
    ...
    for (const ach of ACHIEVEMENTS) {
      const upserted = await (prisma as any).achievement.upsert({
        where:  { name: ach.name },
        update: { description: ach.description, icon: ach.icon, category: ach.category, requirement: ach.requirement, points: ach.points },
        create: ach,
      });
      results.push({ id: upserted.id, name: upserted.name, points: upserted.points });
    }
    res.json({ success: true, seeded: results.length, achievements: results });
  } catch (err: any) {
    console.error('Achievement seed error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// ─── GET version for browser-bar access ──────────────────────────────────────
router.get('/seed-achievements', async (req: Request, res: Response) => {
  req.method = 'POST';
  // Forward to POST handler by re-using the same logic inline
  const secret = req.headers['x-seed-secret'] || req.query.secret;
  if (!secret || secret !== process.env.SEED_SECRET) {
    res.status(401).json({ error: 'Unauthorized' });
    return;
  ...
  const ACHIEVEMENTS = [
    { name: 'First Step',       description: 'Complete your very first workout.',            icon: '🎯', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 1   }, points: 10  },
    { name: 'Getting Started',  description: 'Complete 5 workouts.',                         icon: '🔥', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 5   }, points: 25  },
    { name: 'Committed',        description: 'Complete 10 workouts.',                        icon: '💪', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 10  }, points: 50  },
    { name: 'Dedicated',        description: 'Complete 25 workouts.',                        icon: '⚡', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 25  }, points: 100 },
    { name: 'Unstoppable',      description: 'Complete 50 workouts.',                        icon: '🏆', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 50  }, points: 200 },
    { name: 'Legend',           description: 'Complete 100 workouts.',                       icon: '👑', category: 'MILESTONE', requirement: { type: 'totalWorkouts', value: 100 }, points: 500 },
    { name: 'On a Roll',        description: 'Maintain a 3-day workout streak.',             icon: '📅', category: 'STREAK',    requirement: { type: 'streak', value: 3  }, points: 30  },
    { name: 'Week Warrior',     description: 'Maintain a 7-day workout streak.',             icon: '🗓️', category: 'STREAK',    requirement: { type: 'streak', value: 7  }, points: 75  },
    { name: 'Fortnight Fighter',description: 'Maintain a 14-day workout streak.',            icon: '🌟', category: 'STREAK',    requirement: { type: 'streak', value: 14 }, points: 150 },
    { name: 'Month Master',     description: 'Maintain a 30-day workout streak.',            icon: '🔱', category: 'STREAK',    requirement: { type: 'streak', value: 30 }, points: 400 },
    { name: 'Calorie Crusher',  description: 'Burn 1,000 calories total.',                  icon: '🌡️', category: 'CALORIES', requirement: { type: 'totalCalories', value: 1000  }, points: 50  },
    { name: 'Inferno',          description: 'Burn 5,000 calories total.',                  icon: '🔥', category: 'CALORIES', requirement: { type: 'totalCalories', value: 5000  }, points: 150 },
    { name: 'Furnace',          description: 'Burn 10,000 calories total.',                 icon: '♨️', category: 'CALORIES', requirement: { type: 'totalCalories', value: 10000 }, points: 300 },
    { name: 'Time Invested',    description: 'Log 60 minutes of total workout time.',       icon: '⏱️', category: 'DURATION', requirement: { type: 'totalDuration', value: 60   }, points: 20  },
    { name: 'Hour Power',       description: 'Log 300 minutes of total workout time.',      icon: '⏰', category: 'DURATION', requirement: { type: 'totalDuration', value: 300  }, points: 80  },
    { name: 'Time Lord',        description: 'Log 1,000 minutes of total workout time.',    icon: '🕰️', category: 'DURATION', requirement: { type: 'totalDuration', value: 1000 }, points: 250 },
  ];

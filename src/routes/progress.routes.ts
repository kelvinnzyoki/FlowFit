...
router.post('/achievements/recalculate', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Evaluate each achievement requirement against real data
    for (const ach of allAchievements) {
      let met = false;
      try {
        // requirement is a Json field — could be object or stringified
        const req: any = typeof ach.requirement === 'string'
          ? JSON.parse(ach.requirement as string)
          : ach.requirement;

        // Extract threshold value — support multiple key names used in seed data
        const threshold =
          req.value    ?? req.count  ?? req.target   ??
          req.workouts ?? req.calories ?? req.duration ?? req.streak ?? 0;

        const type = String(req.type || req.field || '').toLowerCase();

        if      (type.includes('workout') || type.includes('count')) met = totalWorkouts  >= threshold;
        else if (type.includes('calor'))                              met = totalCalories  >= threshold;
        else if (type.includes('dur'))                                met = totalDuration  >= threshold;
        else if (type.includes('streak'))                             met = (currentStreak >= threshold || longestStreak >= threshold);
        // Unknown type — skip (don't grant or revoke)
      } catch {
      ...
      if (met && !alreadyUnlocked.has(ach.id)) {
        // Grant: create UserAchievement row (ignore if somehow already exists)
        await prisma.userAchievement.upsert({
          where:  { userId_achievementId: { userId, achievementId: ach.id } },
          create: { userId, achievementId: ach.id },
          update: {}, // already unlocked — don't touch unlockedAt
        });
      }
      // Note: we intentionally do NOT revoke achievements once earned.
      // A user who hit 10 workouts and then deleted some keeps the badge.
    }

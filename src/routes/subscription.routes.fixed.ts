...
// ─── POST /subscriptions/paystack/reconcile-pending/cron ─────────────────────
// Use Vercel Cron or an external scheduler to call this endpoint. setInterval is
// intentionally not used because Vercel serverless instances are stateless.
router.post('/paystack/reconcile-pending/cron', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[paystack/reconcile-pending/cron]', err?.message ?? err);

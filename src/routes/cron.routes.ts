...
// NEW-R5: Daily reconciliation job — scans M-Pesa SUCCESS transactions from the
// last 48h and repairs any drift (missing Payment records, wrong sub status).
// Schedule this once per day in vercel.json, e.g.:
//   { "path": "/internal/cron/reconcile", "schedule": "0 3 * * *" }
router.post('/reconcile', async (req, res) => {
  if (!verifyCronSecret(req, res)) return;

  // Use a 10-minute TTL — reconciliation scans up to 48h of data and may be slow
  // on a large dataset, but must not block indefinitely on Vercel (60s function limit).
  const locked = await acquireCronLock('mpesa-reconcile', 10 * 60 * 1000);
  if (!locked) {
    res.status(409).json({ ok: false, error: 'Job already running' });
    return;
  }
  ...
  try {
    ...
    await releaseCronLock('mpesa-reconcile');
  }
});

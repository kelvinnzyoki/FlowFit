 ...
 *
 * FIX-WH-1  RAW BODY — express.raw() is applied inline on the POST route so the
 *            webhook body is always a Buffer when it reaches verifyPaystackWebhook.
 *            Previously, if express.json() ran first (global middleware), req.body
 *            was a JS object and rawBody.toString('utf8') returned "[object Object]",
 *            causing HMAC to fail on every real Paystack request. That single bug
 *            silently discarded every subscription.create event, leaving
 *            paystackSubscriptionCode and paystackEmailToken permanently NULL.
 *
 * FIX-WH-2  NO ORPHAN ROW CREATION — processChargeSuccess previously created a
 *            brand-new INCOMPLETE subscription row when metaSubscriptionId lookup
 *            missed. When subscription.create fired later it found that newer orphan
 *            row (orderBy createdAt desc) and bound the codes there, leaving the
 *            original INCOMPLETE row stuck forever. Now it logs an error and returns
 *            without creating anything.
 *
 * FIX-WH-3  REFERENCE LOOKUP IN findSubscriptionForCreate — the invoice's
 *            transaction_reference equals the paystackReference stored on the row by
 *            charge.success. That is the most reliable match available but it was
 *            missing from the lookup chain. It is now checked immediately after the
 *            subCode lookup and before the slower customer/email fallbacks.
 *
 * FIX-WH-4  NO STATUS DOWNGRADE IN processChargeSuccess — when charge.success
 *            arrives without codes but the row already has both codes from an earlier
 *            subscription.create, the update no longer forces status back to
 *            INCOMPLETE. It preserves whatever status is already stored.
 */
...
// FIX-WH-3: Added paystackReference lookup — the invoice transaction_reference
// equals the reference stored on our row by charge.success and is the most
// reliable match available. It now runs immediately after the subCode lookup,
// before the slower customer/email fallbacks.
async function findSubscriptionForCreate(data: Record<string, any>) {
  ...
  // Strategy 0: direct ID from checkout metadata — most reliable
  if (metaSubscriptionId) {
    const sub = await prisma.subscription.findUnique({ where: { id: metaSubscriptionId } });
    if (sub) return sub;
  }

  // Strategy 1: already bound to this subscription_code
  if (subCode) {
    const sub = await prisma.subscription.findFirst({ where: { paystackSubscriptionCode: subCode } });
    if (sub) return sub;
  }

  // Strategy 2 (NEW): paystackReference matches invoice transaction_reference.
  // charge.success always stores reference on our row before subscription.create
  // fires, making this the most reliable fallback when metaSubscriptionId is absent.
  const invoiceReference =
    data.most_recent_invoice?.transaction_reference
    ?? data.most_recent_invoice?.reference
    ?? data.reference
    ?? null;
  if (invoiceReference) {
    const sub = await prisma.subscription.findFirst({
      where: { paystackReference: invoiceReference, provider: 'PAYSTACK' },
      orderBy: { createdAt: 'desc' },
    });
    if (sub) return sub;
  }

  // Strategy 3: customer code + plan, INCOMPLETE rows first
  if (customerCode) {
  ...
  // Strategy 4: email fallback — only INCOMPLETE rows to avoid cross-subscription pollution
  if (customerEmail) {
...
async function processChargeSuccess(data: Record<string, any>): Promise<void> {
  ...
  // FIX-WH-2: Removed the previous auto-create block that created a new INCOMPLETE
  // row when all lookups missed. That orphan row was then the newest row for the
  // customer, so subscription.create's "orderBy createdAt desc" lookup bound the
  // Paystack codes to the orphan instead of the original row. Now we fail loudly
  // so the lookup gap is visible in logs, not silently buried in a dead row.
  if (!sub) {
    console.error('[Paystack webhook] charge.success could not match subscription', {
      reference,
      metaSubscriptionId,
      userId,
      planId,
  ...
  // FIX-WH-4: Do not force status to INCOMPLETE if this row already has both
  // codes (subscription.create may have fired and activated before charge.success).
  // Preserve the existing status; only update non-status metadata.
  const alreadyHasCodes = !!sub.paystackSubscriptionCode && !!sub.paystackEmailToken;
  await prisma.subscription.update({
    where: { id: sub.id },
    data: {
      // Preserve status — do NOT downgrade ACTIVE→INCOMPLETE
...
// FIX-WH-1: express.raw() is applied inline on this route so the body is always
// a raw Buffer when it reaches verifyPaystackWebhook. Without this, if express.json()
// runs first (via app.use(express.json()) in server.ts), req.body is a JS object and
// rawBody.toString('utf8') returns "[object Object]" — causing HMAC to mismatch on
// every real Paystack webhook and silently dropping every subscription.create event.

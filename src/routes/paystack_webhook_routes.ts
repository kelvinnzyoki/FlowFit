 ...
 *
 * FIX-8  (Medium) Paystack IP allowlist applied before HMAC verification. Requests
 *        not originating from Paystack's published IP ranges are rejected with 403
 *        before any processing occurs, preventing webhook endpoint abuse/DoS.
 *
...
// Update this list when Paystack publishes new IPs.
...
function isPaystackIp(req: Request): boolean {
  ...
  // req.ip is already resolved by Express when trust proxy is set; fall back to
  // the first XFF entry so both configurations work.
...
async function processChargeSuccess(data: Record<string, any>): Promise<void> {
  ...
  // Locate the subscription row before opening the transaction.
  ...
  // Fetch the plan and compare; allow a 5% rounding tolerance for currency conversion.
  if (amountCents > 0 && sub.planId) {
    ...
    if (plan) {
      const expectedCents = interval === 'YEARLY'
        ? (plan as any).yearlyPriceCents
        : (plan as any).monthlyPriceCents;
      if (
        typeof expectedCents === 'number' &&
        expectedCents > 0 &&
        amountCents < expectedCents * 0.95
      ) {
        console.error(
          `[charge.success FIX-4] Amount mismatch for ref ${reference}: ` +
          `received ${amountCents} cents, expected ${expectedCents} cents (plan ${sub.planId}). ` +
          `Rejecting activation — check for webhook tampering or test-mode bleed.`,
        );
        return; // Do NOT activate; do NOT record a succeeded payment.
      }
    }
  }

  if (subscriptionCode && emailToken) {
    // FIX-14: Wrap payment creation + subscription activation in a single transaction
    // so a mid-operation crash cannot leave one done and the other skipped.
  ...
  // FIX-14: Also wrap the partial-update path in a transaction.
  await prisma.$transaction(async (tx) => {
    if (amountCents > 0) {
      const existingPayment = await tx.payment.findFirst({ where: { paystackReference: reference } });
      if (!existingPayment) {
...
//
// FIX-8: isPaystackIp() rejects non-Paystack IPs with 403 before any crypto work,
// preventing the endpoint from being used for DoS/invocation abuse.
router.post('/', express.raw({ type: 'application/json' }), async (req: Request, res: Response) => {
  // FIX-8: IP allowlist check
  if (!isPaystackIp(req)) {
    console.warn(`[Paystack webhook] Rejected request from non-Paystack IP: ${req.ip}`);

...
async function updateStreak(userId: string) {
  try {
    // Use explicit UTC midnight dates so user timezone never affects streak logic.
    // setHours(0,0,0,0) uses the SERVER's local timezone (UTC on Vercel), which
    // causes users in UTC+ timezones to have workouts logged on the "wrong" UTC day
    // relative to their local calendar — breaking consecutive-day detection and
    // silently resetting streaks to 1.
    const now = new Date();
    ...
    // Normalise lastWorkoutDate to UTC midnight for a timezone-safe comparison

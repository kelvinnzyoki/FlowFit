...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    // FIX: Paystack fires subscription.disable in two scenarios:
    //   A) Period-end cancel: we called paystackDisable earlier (non-renewing),
    //      period has now passed, Paystack fires this to confirm full cancellation.
    //      → Set status=CANCELLED only if currentPeriodEnd has passed.
    //   B) Immediate cancel: we called paystackDisable for immediate=true.
    //      → Set status=CANCELLED immediately (period end irrelevant).
    //
    // Previously the handler unconditionally set status=CANCELLED on every
    // subscription.disable event, causing users who chose "Cancel at Period End"
    // to lose access immediately when the non-renewing webhook arrived.
    case 'subscription.disable': {
      ...
      // If cancelAtPeriodEnd=true and the billing period has NOT ended yet,
      // the user is still entitled to access. Don't cancel — just ensure the
      // DB reflects the scheduled cancellation state.
      const periodEndDate   = sub.currentPeriodEnd ? new Date(sub.currentPeriodEnd) : null;
      const periodHasEnded  = !periodEndDate || periodEndDate.getTime() <= now.getTime();
      const scheduledCancel = sub.cancelAtPeriodEnd === true;

      if (scheduledCancel && !periodHasEnded) {
        // Period hasn't ended — keep ACTIVE, just mark cancelAtPeriodEnd in case
        // it wasn't already set (e.g. webhook arrived before our DB update).
        await prisma.subscription.update({
          where: { id: sub.id },
        ...
        await prisma.subscriptionLog.create({
          data: {
            ...
            metadata: {
              ...
              note: 'webhook_subscription.disable_before_period_end',
      ...
      // Period has ended OR this was an immediate cancel — fully cancel now.
      await prisma.$transaction(async (tx) => {
        await tx.subscription.update({
          where: { id: sub.id },
          data: {
      ...
      const planName  = (sub as any).plan?.name ?? 'your plan';
      const endDateFmt = periodEndDate ? periodEndDate.toLocaleDateString() : '';
      notifySubCancelled(sub.userId, planName, endDateFmt, !scheduledCancel)

...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    case 'subscription.create': {
      ...
      // Note: metadata is NOT present in subscription.create — only in charge.success
      // Paystack's subscription.create payload does not include a transaction
      // reference field directly. Try all known paths across API versions:
      //   data.most_recent_invoice.reference (v2 webhooks)
      //   data.most_recent_invoice.transaction_reference (older format)
      //   data.invoice.reference (alternate shape)
      // If none are found, strategy 2 (email → user lookup) handles matching.
      const reference = (
      ...
      // Lookup strategy 1: match by paystackReference stored at checkout.
      // The row may be INCOMPLETE (charge.success hasn't fired yet) or
      // ACTIVE (charge.success already ran). Include both.
      let dbSub = reference
      ...
      // Lookup strategy 2: match by customer email → user
      if (!dbSub && custEmail) {
      ...
      // Lookup strategy 3: find any ACTIVE sub that already has this code
      // (charge.success may have set it already — just ensure emailToken is stored)
      if (!dbSub) {
        dbSub = await prisma.subscription.findFirst({
      ...
      // (email_token is only guaranteed in subscription.create, not charge.success)

...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    case 'subscription.create': {
      ...
      const reference  = data.most_recent_invoice?.transaction?.reference as string | undefined;
      ...
      let dbSub = reference
        ? await prisma.subscription.findFirst({
            where:   { paystackReference: reference, status: { in: ['INCOMPLETE', 'ACTIVE'] } },
            orderBy: { createdAt: 'desc' },
          })
        : null;
      ...
      if (!dbSub && custEmail) {
        ...
        if (user) {
          dbSub = await prisma.subscription.findFirst({
            where:   { userId: user.id, status: { in: ['INCOMPLETE', 'ACTIVE'] } },
            orderBy: { createdAt: 'desc' },
          });
        }
      }

...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    case 'charge.success': {
      ...
      // For first payments, charge.success often arrives before subscription.create
      // and therefore does not include subscription_code/email_token. Try to resolve
      // them synchronously from Paystack now. If they still cannot be stored, keep
      // the DB subscription INCOMPLETE. The website must not unlock until both codes
      // exist because billing management depends on them.
      if (existing && (existing.status === 'INCOMPLETE' || existing.status === 'TRIALING') && (!subscriptionCode || !emailToken)) {
        const resolved = await resolveAndStorePaystackSubscriptionCodes({
          subscriptionId: existing.id,
          txn: data,
          attempts: 4,
          delayMs: 2_000,
        }).catch((err: any) => {
          console.warn('[Webhook] charge.success: could not resolve codes before activation:', err?.message ?? err);
          return null;
        });

        if (resolved?.subscription_code && resolved.email_token) {
          subscriptionCode = resolved.subscription_code;
          emailToken       = resolved.email_token;
          if (resolved.next_payment_date) nextPaymentDate = new Date(resolved.next_payment_date);
        }
      }
      ...
      await prisma.$transaction(async (tx) => {
        if (existing && (existing.status === 'INCOMPLETE' || existing.status === 'TRIALING')) {
          ...
          await tx.subscription.update({
            ...
            data: {
              status:                   subscriptionCode && emailToken ? 'ACTIVE' : 'INCOMPLETE',
              provider:                 'PAYSTACK',
              interval,
              ...
              activatedAt:              subscriptionCode && emailToken ? now : undefined,
          ...
          await tx.subscriptionLog.create({
            data: {
              ...
              newStatus:      subscriptionCode && emailToken ? 'ACTIVE' : 'INCOMPLETE',
              metadata: {
                paystackReference: reference,
                note: subscriptionCode && emailToken
                  ? 'first_payment_codes_stored_and_activated'
                  : 'payment_succeeded_waiting_for_subscription_codes',
              },
            },
          });
          ...
          if (!sub) {
            ...
            const created = await tx.subscription.create({
              data: {
                ...
                currentPeriodStart:       now,
                currentPeriodEnd:         nextPaymentDate,
                activatedAt:              subscriptionCode && emailToken ? now : null,
              },
            });
            await tx.payment.create({
              data: {
            ...
            await tx.subscriptionLog.create({
              data: {
                ...
                metadata: {
                  paystackReference: reference,
                  note: subscriptionCode && emailToken
                    ? 'created_from_webhook_fallback_codes_stored'
                    : 'created_from_webhook_fallback_waiting_for_codes',
                },
              },
            });
            return;
      ...
      if (!isRenewal && existing && subscriptionCode && emailToken) {

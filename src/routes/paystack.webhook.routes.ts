...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    case 'subscription.create': {
      ...
      // transaction metadata (userId, planId). The most_recent_invoice.transaction
      // field is a transaction CODE string (e.g. "TRN_xxx"), not an object with a
      // reference. The payment reference we stored is NOT reliably available here.
      // Strategy 1 (reference match) is skipped — Strategy 2 (email) is primary.
      if (!subCode) return;
      ...
      // Lookup strategy 3: most recently created INCOMPLETE row (last resort)
      if (!dbSub) {
        dbSub = await prisma.subscription.findFirst({
          where:   { status: 'INCOMPLETE', provider: 'PAYSTACK' },
          orderBy: { createdAt: 'desc' },
        });
      }

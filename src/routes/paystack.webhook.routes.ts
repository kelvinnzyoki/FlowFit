...
function formatWebhookPlanName(plan: any): string {
  return plan?.name || plan?.slug || 'your plan';
}

function formatWebhookDate(date: Date | null | undefined): string {
  ...
  switch (event.event) {
    ...
    case 'subscription.disable': {
      ...
      const freshSub = await prisma.subscription.findUnique({
        where: { id: sub.id },
        select: { cancelAtPeriodEnd: true, scheduledPlanId: true, scheduledInterval: true, cancellationReason: true },
      });
      ...
      const effectiveCancellationReason =
        freshSub?.cancellationReason ?? sub.cancellationReason;
      const isScheduledDowngrade =
        !!effectiveScheduledPlanId || effectiveCancellationReason === 'scheduled_downgrade';

      const scheduledPlan = effectiveScheduledPlanId
        ? await prisma.plan.findUnique({ where: { id: effectiveScheduledPlanId }, select: { name: true, slug: true } })
        : null;
      ...
      const isPeriodEndCancel = periodStillActive && (effectiveCancelAtPeriodEnd === true || isScheduledDowngrade);
      ...
      if (isPeriodEndCancel) {
        ...
        if (isScheduledDowngrade) {
          await notifySubscriptionAction(
            ...
            'Downgrade scheduled',
            `${formatWebhookPlanName((sub as any).plan)} will change to ${formatWebhookPlanName(scheduledPlan)} on ${formatWebhookDate(sub.currentPeriodEnd)}.`,
            '⬇️',
          );
        } else {
    ...
    case 'subscription.not_renew': {
      ...
      const scheduledPlan = sub.scheduledPlanId
        ? await prisma.plan.findUnique({ where: { id: sub.scheduledPlanId }, select: { name: true, slug: true } })
        : null;
      const isScheduledDowngrade = !!sub.scheduledPlanId || sub.cancellationReason === 'scheduled_downgrade';
      ...
      if (isScheduledDowngrade) {
        await notifySubscriptionAction(
          ...
          'subscription_downgrade_scheduled',
          'Downgrade scheduled',
          `${formatWebhookPlanName((sub as any).plan)} will change to ${formatWebhookPlanName(scheduledPlan)} on ${endDateStr || formatWebhookDate(sub.currentPeriodEnd)}.`,
          '⬇️',
        );
      } else {
        notifySubCancelled(sub.userId, (sub as any).plan?.name ?? 'your plan', endDateStr, false)

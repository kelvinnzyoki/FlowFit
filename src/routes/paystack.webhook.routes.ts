...
async function processChargeSuccess(data: Record<string, any>): Promise<void> {
  ...
  // Paystack amounts are in kobo. Plan prices in DB must also be stored in kobo
  // (e.g. KES 1,200/month = 120000). Verify with:
  // SELECT monthly_price_cents FROM plans WHERE slug='pro' — expected: 120000.
...
router.post('/', express.raw({ type: 'application/json' }), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (createErr.code === 'P2002' || createErr.message?.includes('Unique constraint')) {
      ...
      res.json({ received: true, idempotencyFailed: true });
      return;
    }
  }

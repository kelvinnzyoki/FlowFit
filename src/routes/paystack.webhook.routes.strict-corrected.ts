...
import {
  ...
  extractPaystackManagementCodes,
  fetchAndStorePaystackCodes,
...
async function processEvent(event: PaystackWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    ...
    case 'charge.success': {
      ...
      // Extract any management codes Paystack included. First-payment
      // charge.success often does NOT include email_token, so this event must
      // not activate a subscription unless both fields are present or can be
      // fetched synchronously from Paystack.
      const extractedCodes = extractPaystackManagementCodes(data as Record<string, any>);
      let subscriptionCode: string | undefined = extractedCodes.subscriptionCode;
      let emailToken: string | undefined = extractedCodes.emailToken;
      ...
      const customerCode =
        (data.customer as any)?.customer_code ||
        (meta.customerCode as string | undefined) ||
        existing?.paystackCustomerCode ||
        undefined;

      // STRICT GATE: first payment must not become ACTIVE until both
      // paystackSubscriptionCode and paystackEmailToken are stored.
          ...
          await prisma.subscriptionLog.create({
            data: {
              ...
              metadata: {
                ...
                reason: 'missing_paystackSubscriptionCode_or_paystackEmailToken',
              },
            },
          }).catch(err => console.warn('[Webhook] charge.success pending log failed:', err));

          console.warn(
            `[Webhook] charge.success: payment received but activation blocked for sub ${existing.id}; ` +
            'waiting for subscription.create to provide management codes.',
          );
          return;
        }

        subscriptionCode = codeResult.subscriptionCode;
        emailToken       = codeResult.emailToken;
        if (codeResult.nextPaymentDate) nextPaymentDate = codeResult.nextPaymentDate;
      }

      await prisma.$transaction(async (tx) => {
        if (existing && (existing.status === 'INCOMPLETE' || existing.status === 'TRIALING')) {
          // ── First payment: activate only after both Paystack management
          // codes are confirmed. The strict gate above guarantees these are set.
          ...
          if (!subscriptionCode || !emailToken) {
            console.warn(`[Webhook] charge.success: activation skipped inside transaction for sub ${existing.id}; missing codes.`);
            return;
          }

          console.log(`[Webhook] charge.success: activating sub ${existing.id} after codes confirmed`, {
            reference,
            subscriptionCode,
            hasEmailToken: true,
          });
          ...
          await tx.subscriptionLog.create({
            data: {
              ...
              previousStatus: existing.status,
              newStatus:      'ACTIVE',
              metadata: { paystackReference: reference, note: 'first_payment_charge_success_codes_confirmed' },
            },
          });
          ...
          if (!sub) {
            ...
            if (!subscriptionCode || !emailToken) {
              console.warn('[Webhook] charge.success renewal/fallback: not creating ACTIVE sub without subscriptionCode and emailToken. reference:', reference);
              return;
            }
            console.warn(
          ...
          emailToken = emailToken ?? sub.paystackEmailToken ?? undefined;
          subscriptionCode = subscriptionCode ?? sub.paystackSubscriptionCode ?? undefined;
          if (!subscriptionCode || !emailToken) {
            console.warn('[Webhook] charge.success renewal: refusing ACTIVE update because management codes are missing.', { subId: sub.id, reference });
            return;
          }

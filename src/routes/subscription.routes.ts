...
router.get('/billing-portal', requireAuth, billingLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // FIX: Paystack manage/subscriptions URL uses the email_token.
    // This is the standard Paystack self-service link that lets customers
    // update their card, view invoices, and cancel their subscription.
    if (sub?.paystackEmailToken) {
      return res.json({
        success: true,
        url:     `https://paystack.com/manage/subscriptions/${sub.paystackEmailToken}`,

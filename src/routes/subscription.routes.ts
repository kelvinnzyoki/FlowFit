...
const paystackReconcileGlobal = globalThis as typeof globalThis & {
  __flowfitPaystackReconcileInterval?: ReturnType<typeof setInterval>;
};

if (!paystackReconcileGlobal.__flowfitPaystackReconcileInterval) {
  paystackReconcileGlobal.__flowfitPaystackReconcileInterval = setInterval(async () => {
    try {
      const result = await reconcilePendingPaystackSubscriptions(25);
      if (result.activated > 0) {
        console.log('[cron] reconcilePendingPaystack:', result.message);
      }
    } catch (err) {
      console.error('[cron] reconcilePendingPaystack failed:', err);
    }
  }, 10 * 60 * 1000);

  paystackReconcileGlobal.__flowfitPaystackReconcileInterval.unref?.();
}
...
router.get('/billing-portal', requireAuth, billingLimiter, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: err.message ?? 'Failed to open Paystack billing management' });
  }
});
...
router.post('/paystack/reconcile-pending', requireAuth, billingLimiter, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: err.message ?? 'Paystack reconciliation failed' });
  }
});

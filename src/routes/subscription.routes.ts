...
function validateCallbackUrl(url: string | undefined): string | undefined {
  ...
  // Build a set of all allowed hostnames from every configured origin env var.
  // This handles the common case where FRONTEND_URL is the app subdomain
  // (flowfit.cctamcc.site) while APP_URL / API_URL is the API subdomain
  // (fit.cctamcc.site) — both must be trusted for callback URLs.
  const rawOrigins = [
    process.env.FRONTEND_URL,
    process.env.APP_URL,
    process.env.CORS_ORIGIN,      // may be comma-separated list
  ]
    .flatMap(o => (o ?? '').split(','))
    .map(o => o.trim())
    .filter(Boolean);
  ...
  if (rawOrigins.length === 0) {
    // No env config — development mode, allow any URL
    console.warn('[subscription] No FRONTEND_URL/APP_URL set — accepting any callbackUrl in dev');
    return url;
  }
  ...
  try {
    ...
    if (allowedHostnames.has(parsed.hostname)) return url;
    console.warn(
      `[subscription] Blocked callbackUrl hostname "${parsed.hostname}". ` +
      `Allowed: ${[...allowedHostnames].join(', ')}. ` +
      `Add the correct hostname to FRONTEND_URL in your Vercel env vars.`,
    );
    return undefined;
  } catch {
    return undefined;
...
//
// FIX: Paystack has no hosted billing portal, but it does provide a per-subscription
// manage URL: https://paystack.com/manage/subscriptions/{emailToken}
// This lets users update their card on file directly on Paystack's page.
//
// If the user has an active Paystack subscription with a paystackEmailToken,
// return the Paystack manage URL. Otherwise return the in-app subscription page.
router.get('/billing-portal', requireAuth, billingLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // FIX: Removed `paystackEmailToken: { not: null }` filter.
    // Previously this made `sub` null for any subscription without an emailToken in
    // the DB (common when subscription.create webhook was missed). The next line then
    // crashed with "Cannot read properties of null" → 500 → billing portal broken.
    // Also added `id: true` to the select so the update call below has a valid id.
    const sub = await prisma.subscription.findFirst({
      ...
      orderBy: { createdAt: 'desc' },
      select: {
        id:                       true,   // FIX: was missing — caused update to use undefined id
    ...
    // FIX: null check before accessing sub properties
    let emailToken = sub?.paystackEmailToken ?? null;

    // If emailToken is missing (subscription.create webhook was missed),
    // fetch it live from Paystack using the subscription code.
    if (!emailToken && sub?.paystackSubscriptionCode) {
      try {
        const { fetchPaystackSubscription } = await import('../services/paystack.service.js');
        ...
        if (emailToken) {
          ...
          }).catch(() => {/* non-critical — persist for next call */});
        }
      } catch (err) {
        console.error('[billing-portal] Failed to fetch emailToken from Paystack:', err);
      }
    }
    ...
    if (emailToken) {
      return res.json({
        ...
        note:    'Opens Paystack subscription management.',
      });
    }

    // Active Paystack sub but no emailToken retrievable — guide user in-app
    if (sub) {
      return res.json({
        success: true,
        url:     `${appUrl}/subscription.html`,
        note:    'Manage your subscription on this page.',
      });
    }

    // M-Pesa subscription — no external portal
    const mpesaSub = await prisma.subscription.findFirst({
      where:  { userId, provider: 'MPESA', status: { in: ['ACTIVE', 'TRIALING', 'PAST_DUE'] } },
      select: { id: true },
    });
    if (mpesaSub) {
      return res.json({
        success: true,
        url:     `${appUrl}/subscription.html`,
        note:    'M-Pesa subscriptions are managed in-app.',
      });
    }

    // Free user or no active subscription
    return res.json({
      success: true,
      url:     `${appUrl}/subscription.html`,
      note:    'No active subscription. Choose a plan to get started.',
    });
  } catch (err: any) {
    console.error('[billing-portal] error:', err);
    res.status(500).json({ error: err.message ?? 'Failed to get billing portal' });
  }
});

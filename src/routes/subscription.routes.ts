...
function toClientSubscription(subscription: any) {
  if (!subscription) return subscription;
  const { paystackSubscriptionCode, paystackEmailToken, ...safe } = subscription;
  return safe;
}
      ...
      if (result.status === 'pending_subscription_codes' || result.pending) {
        ...
        if (reconciled.activated > 0 && reconciled.subscription?.status === 'ACTIVE') {
          res.json({
            ...
            pending: false,
            status: 'success',
            subscription: toClientSubscription(reconciled.subscription),
        ...
        res.status(202).json({
          ...
          subscription: toClientSubscription(reconciled.subscription ?? result.subscription ?? null),
          reconcile: {
    ...
    try {
      ...
      if (sub.paystackSubscriptionCode && sub.paystackEmailToken && sub.status === 'ACTIVE') {
        res.json({
          ...
          pending: false,
          fetched: false,
          subscription: toClientSubscription(sub),
      ...
      res.json({
        ...
        pending: false,
        fetched: true,
        subscription: toClientSubscription(updatedSub),

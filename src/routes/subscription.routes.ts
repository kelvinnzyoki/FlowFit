...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // tx.subscriptionId is nullable — if null, no user can own it
      if (!tx.subscriptionId) {
        res.status(403).json({ error: 'Forbidden' });
        return;
      }
      const sub = await prisma.subscription.findUnique({
        where: { id: tx.subscriptionId },   // string (null checked above)
        select: { userId: true },
      });
      ...
      if (darajaResult.resultCode !== '17') {
        ...
        if (darajaResult.resultCode === '0') {
          ...
          // Terminal failure — update DB directly (handleMpesaFailure is not exported)

...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Block if currently active or trialing
      ...
      // FIX-M3: Block if the user has EVER had a trial (not just currently).
      // A user who completed, cancelled, then expired a trial could previously
      // restart the trial indefinitely. One trial per account.
      ...
      if (previousTrial) {
        res.status(400).json({
          error: 'Trial already used — each account is eligible for one free trial only',
        });
        return;
      }

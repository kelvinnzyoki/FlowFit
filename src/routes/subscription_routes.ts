...
router.get('/billing-portal', requireAuth, billingLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (err.message === 'No billing account found') {
      res.status(400).json({
        error: 'mpesa_no_billing_portal',
        message: 'Billing portal is only available for Stripe subscriptions. Your M-Pesa subscription is managed directly on the subscription page.',
      });
      return;
    }
...
router.post('/trial', requireAuth, async (req: Request, res: Response) => {
  try {
    const { planId } = req.body;
    if (!planId || typeof planId !== 'string') {
      res.status(400).json({ error: 'planId is required' }); return;
    }
    const subscription = await startTrial(req.user!.id, planId, req.ip ?? undefined);
    res.json({ subscription });
  } catch (err: any) {
    res.status(400).json({ error: err.message ?? 'Could not start trial' });
  }
});

// ─── GET /subscriptions/payments ─────────────────────────────────────────────
// Returns payment records for the current user so M-Pesa subscribers can view
// their payment history without needing the Stripe billing portal.
router.get('/payments', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;

    // Fetch from both Stripe payments (via subscription logs) and M-Pesa transactions
    const [payments, mpesaTxns] = await Promise.all([
    ...
    const stripePayments = payments.map(p => ({
      id:           p.id,
      ...
      via:          'stripe',
      stripeInvoiceId: (p as any).stripeInvoiceId ?? null,
      mpesaReceipt: null,
    }));

    const mpesaPayments = mpesaTxns.map(t => ({
      id:           t.id,
      createdAt:    t.createdAt,
      amountCents:  Math.round((t.amountKes / parseFloat(process.env.MPESA_EXCHANGE_RATE ?? '130')) * 100),
      currency:     'KES',
      status:       'succeeded',
    ...
    const all = [...stripePayments, ...mpesaPayments]
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

    res.json({ payments: all });
  } catch (err: any) {
    res.status(500).json({ error: err.message ?? 'Could not fetch payments' });
  }
});
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (queryResult.ResultCode === '0') {
        ...
        // Map Safaricom result codes to user-friendly messages
        const RETRIABLE_CODES = new Set([
          '1032', // Request cancelled by user — let them retry
          '1037', // DS timeout — sandbox-only: simulated device unreachable, not a real failure
          '1001', // Insufficient funds — user should fix balance and retry
        ]);
        const ERROR_MESSAGES: Record<string, string> = {
          '1032': 'You cancelled the M-Pesa request. Tap RETRY to try again.',
          '1037': 'M-Pesa could not reach your phone. Please ensure your phone is on and retry.',
          '1001': 'Insufficient M-Pesa balance. Top up and retry.',
          '2001': 'Wrong M-Pesa PIN entered. Tap RETRY to try again.',
        };
        const isRetriable = RETRIABLE_CODES.has(queryResult.ResultCode);
        const userMessage = ERROR_MESSAGES[queryResult.ResultCode]
          || queryResult.ResultDesc
          || 'Payment was not completed. Please try again.';

        if (!isRetriable) {
          // Permanent failure — mark in DB so we stop polling
          await prisma.mpesaTransaction.update({
            where: { checkoutRequestId },

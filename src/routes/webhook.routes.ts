...
async function processEvent(event: Stripe.Event): Promise<void> {
  switch (event.type) {
    ...
    case 'checkout.session.completed': {
      ...
      // ── Notify user after checkout ────────────────────────────────────────
      // Fire-and-forget: never block the webhook response on notification errors.
      // Look up planName from planId (provided in session metadata).
      (async () => {
        try {
          ...
          if (newStatus === 'TRIALING') {
            await notifyTrialStarted(userId, planName, plan?.trialDays ?? 7);
          } else {
            await notifySubActivated(userId, planName, 'STRIPE');
          }
        } catch (err) {
          console.error('[Webhook] checkout notification failed:', err);
        }
      })();
    ...
    case 'invoice.payment_succeeded': {
      ...
      // ── Notify renewal ────────────────────────────────────────────────────
      const renewedPlanName  = (sub as any).plan?.name ?? 'your plan';
      ...
      notifySubRenewed(sub.userId, renewedPlanName, nextBillingDate)
    ...
    case 'invoice.payment_failed': {
      ...
      const failedPlanName = (sub as any).plan?.name ?? 'your plan';
      notifyPaymentFailed(sub.userId, failedPlanName)
    ...
    case 'customer.subscription.updated': {
      ...
      // ── Notify on key state transitions ───────────────────────────────────
      const updatedPlanName = (sub as any).plan?.name ?? 'your plan';
      if (trialConverted) {
        notifySubActivated(sub.userId, updatedPlanName, 'STRIPE')
          .catch(err => console.error('[Webhook] trial_converted notification failed:', err));
      } else if (stripeSub.cancel_at_period_end && !sub.cancelAtPeriodEnd) {
        // Cancellation was just scheduled this event
        const endDateStr = new Date(stripeSub.current_period_end * 1000)
          .toLocaleDateString('en-KE', { day: 'numeric', month: 'long', year: 'numeric' });
        notifySubCancelled(sub.userId, updatedPlanName, endDateStr, false)
          .catch(err => console.error('[Webhook] cancellation_scheduled notification failed:', err));
      }
    ...
    case 'customer.subscription.deleted': {
      ...
      // ── Notify immediate cancellation ─────────────────────────────────────
      const deletedPlanName = (sub as any).plan?.name ?? 'your plan';
      notifySubCancelled(sub.userId, deletedPlanName, '', true)
        .catch(err => console.error('[Webhook] sub_deleted notification failed:', err));
    ...
    case 'customer.subscription.trial_will_end': {
      ...
      // ── Notify trial ending (replaces TODO) ───────────────────────────────
      // Stripe fires this event 3 days before trial ends.
      // Calculate precise daysLeft from the trial_end timestamp.
      if (stripeSub.trial_end) {
        const msLeft  = stripeSub.trial_end * 1000 - Date.now();
        const daysLeft = Math.max(1, Math.ceil(msLeft / 86_400_000));
        const trialPlanName = (sub as any).plan?.name ?? 'your plan';
        // include plan if not already included on this sub object
        const planName = trialPlanName !== 'your plan'
          ? trialPlanName
          : (await prisma.plan.findUnique({ where: { id: sub.planId }, select: { name: true } }))?.name ?? 'Premium';

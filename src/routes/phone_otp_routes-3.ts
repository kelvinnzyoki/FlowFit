...
// FIX: AT sandbox REQUIRES the literal string 'sandbox' as username.
// Using any other value (e.g. your production app name) causes AT to return
// 401 Unauthorized against the sandbox URL — which becomes a 500 from FlowFit.
// In production, use the app-specific username from the AT dashboard.
const AT_USERNAME = IS_PROD
...
if (IS_PROD && !process.env.AFRICASTALKING_USERNAME) console.warn('[phone-otp] AFRICASTALKING_USERNAME is not set in production — SMS delivery will fail');

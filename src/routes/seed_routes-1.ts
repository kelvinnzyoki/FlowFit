...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // FIX TS2322: Program schema requires userId (not optional).
    // Seed programs are global templates — we attach them to the first admin
    // user found, or the first user in the DB. If no users exist yet the loop
    // is skipped; programs are seeded again on the next /seed call after the
    ...
    if (!seedUser) {
      results.push({ type: 'programs', count: 0, note: 'No users in DB yet — programs will be seeded after first registration.' });
    }

    let progCount = 0;
    for (const p of programs) {
      if (!seedUser) break; // skip program seeding until a user exists

      await prisma.program.upsert({
        where:  { id: p.id },
        update: { name: p.title, description: p.description, durationWeeks: p.durationWeeks, daysPerWeek: p.daysPerWeek },
        // FIX: pass userId so Prisma resolves to ProgramUncheckedCreateInput
        create: { id: p.id, name: p.title, description: p.description, durationWeeks: p.durationWeeks, daysPerWeek: p.daysPerWeek, userId: seedUser.id },
      });
      ...
      for (let wi = 0; wi < p.weeks.length; wi++) {
        ...
        for (let di = 0; di < p.weeks[wi].length; di++) {
          ...
          for (let ei = 0; ei < p.weeks[wi][di].length; ei++) {
            ...
            await prisma.programDayExercise.upsert({
              where:  { id: deId },
    ...
    // ── Plans ──────────────────────────────────────────────────────────────────
    // Free, Pro, and Elite — matched to your Paystack dashboard plan codes.
    // Set PAYSTACK_PLAN_CODE_PRO_MONTHLY etc. in your env vars, or replace the
    // env var references below with the literal plan codes from your Paystack
    // dashboard (e.g. 'PLN_xxxxxxxxxxxxxxx').
    const plans = [
      {
        id:          'plan-free',
        slug:        'free',
        name:        'Free',
        description: 'Get started with FlowFit at no cost. Perfect for beginners exploring fitness tracking.',
        // Prices stored in cents (1 KES = 100 cents)
        monthlyPriceCents: 0,
        yearlyPriceCents:  0,
        mpesaMonthlyKes:   0,
        mpesaYearlyKes:    0,
        // Free plan has no Paystack plan codes
      ...
      },
      {
        ...
        slug:        'pro',
        name:        'Pro',
        description: 'Unlock unlimited workouts, advanced analytics, and premium training programs.',
        monthlyPriceCents: 149900,   // KES 1,499 / month
        yearlyPriceCents:  1439000,  // KES 14,390 / year  (~20% off)
        mpesaMonthlyKes:   1499,
        mpesaYearlyKes:    14390,
        // Set these in your Paystack dashboard and copy the plan codes here
        paystackPlanCodeMonthly: process.env.PAYSTACK_PLAN_CODE_PRO_MONTHLY ?? null,
        paystackPlanCodeYearly:  process.env.PAYSTACK_PLAN_CODE_PRO_YEARLY  ?? null,
        trialDays:            7,
        maxWorkoutsPerMonth:  null,  // unlimited
        maxPrograms:          null,  // unlimited
      ...
      },
      {
        ...
        slug:        'elite',
        name:        'Elite',
        description: 'The ultimate FlowFit experience. Personal coaching, offline access, and every premium feature.',
        monthlyPriceCents: 299900,   // KES 2,999 / month
        yearlyPriceCents:  2879000,  // KES 28,790 / year  (~20% off)
        mpesaMonthlyKes:   2999,
        mpesaYearlyKes:    28790,
        paystackPlanCodeMonthly: process.env.PAYSTACK_PLAN_CODE_ELITE_MONTHLY ?? null,
        paystackPlanCodeYearly:  process.env.PAYSTACK_PLAN_CODE_ELITE_YEARLY  ?? null,
        trialDays:            7,
        maxWorkoutsPerMonth:  null,  // unlimited
        maxPrograms:          null,  // unlimited
        hasAdvancedAnalytics: true,
    ...
    for (const plan of plans) {
      await prisma.plan.upsert({
        ...
        update: {
          ...
          mpesaMonthlyKes:         plan.mpesaMonthlyKes,
          mpesaYearlyKes:          plan.mpesaYearlyKes,
          paystackPlanCodeMonthly: plan.paystackPlanCodeMonthly,
          paystackPlanCodeYearly:  plan.paystackPlanCodeYearly,
    ...
    log.push(`${planCount} plans (free, pro, elite)`);

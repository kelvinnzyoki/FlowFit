// SECURITY PATCHES APPLIED: FIX-1
/**
 * FLOWFIT — Subscription Types (v4)
 *
...
export interface CurrentSubscription {
   ...
   *
   * Used server-side only to call:
   *   POST https://api.paystack.co/subscription/disable
   *   POST https://api.paystack.co/subscription/enable
   * paystackEmailToken (the second auth factor for those calls) is intentionally
   * NOT included here — it is a backend-only credential and must never be sent
   * to the client.
   */
  paystackSubscriptionCode: string | null;
  hasPaystackCodes: boolean;

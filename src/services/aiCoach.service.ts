...
Never invent data. Always reference real numbers from context. "response" is always required.`;
  ...
  private getGoalWorkoutBlueprint(ctx: UserContext): { title: string; focus: string; exercises: { name: string; sets: number; reps: string; note?: string }[] } {
    ...
    const recentDone = ctx.recentLogs.filter(l => !l.skipped).length;
    const rotationBase = new Date().getDay();          // 0–6: changes daily
    const sessionOffset = recentDone % 3;              // 0–2: advances each logged session
...
• Protein target today: ${protein}g`;
  ...
  private generateWorkout(ctx: UserContext): CoachResponse {
    ...
    if (!programCtx || !enrollment?.program) {

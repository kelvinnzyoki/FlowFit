...
class AICoachService {
  ...
  private async callLLM(
    ...
    const systemPrompt = `You are FlowFit's elite AI personal coach — data-driven, warm, direct, and specific.
You have the athlete's REAL data. Use it. Never give generic advice when you have specifics.

PRINCIPLES:
- Reference real data: their program name, recent sessions by exercise, streak number, weight progress
- Be specific: "Your pull volume dropped 22% this week" not "keep training hard"
- Tone: knowledgeable friend who trains hard — encouraging but honest, never sycophantic
- Fatigue > 6: steer toward recovery, not more volume
- Adherence < 60: prioritise consistency over intensity
- In a program: guide through today's session by exercise name
- Celebrate real milestones with specific numbers
- Coaching enabled: ${ctx.hasCoaching}
- Nutrition enabled: ${ctx.hasNutrition}

RESPONSE — always valid JSON:
{
  "intent": "<what user wants>",
  "tool": { "name": "<tool>", "args": {} },
  "response": "<coaching reply — specific, motivating, max 3 short paragraphs>"
}
...
Never invent data. "response" is always required.`;
    ...
    const res = await fetchWithRetry(GROQ_CHAT_URL, {
      ...
      body: JSON.stringify({
        model: GROQ_MODEL, temperature: 0.45, max_completion_tokens: 1200,
    ...
    const result = LLMResponseSchema.safeParse(safeJsonParse(raw));
    if (!result.success) {
      console.warn('[AICoach] validation failed:', result.error.message);
      return { intent: 'unknown', response: raw.slice(0, 600) };
    }
    return result.data;
  }

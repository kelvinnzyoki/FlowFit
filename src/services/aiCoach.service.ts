...
${generated.reply}`,
  ...
  if (!dayEx.length) {
    return { success: true, reply: `Couldn't load today's exercises for **${ctx.program.name}**. Check the Programs page.` };
  }
  ...
  return {
    success: true,
    reply,
    data: { exercises: dayEx.map(de => de.exercise).filter(Boolean), week: ctx.program.currentWeek, day: ctx.program.currentDay },
  };
  }

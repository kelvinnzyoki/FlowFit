...
function normalizeRegion(input?: string | null): AthleteRegion {
  ...
  if (raw.includes('north') || raw.includes('canada') || raw.includes('usa') || raw.includes('america/new') || raw.includes('america/los')) return 'North America';
  if (raw.includes('south') || raw.includes('brazil') || raw.includes('argentina') || raw.includes('sao_paulo') || raw.includes('buenos')) return 'South America';
  if (raw.includes('europe') || raw.includes('london') || raw.includes('berlin') || raw.includes('paris') || raw.includes('rome')) return 'Europe';
  if (raw.includes('asia') || raw.includes('tokyo') || raw.includes('kolkata') || raw.includes('singapore') || raw.includes('dubai')) return 'Asia';
  if (raw.includes('australia') || raw.includes('sydney') || raw.includes('melbourne') || raw.includes('oceania')) return 'Australia';
  return VALID_REGIONS.find(r => r.toLowerCase() === raw) ?? 'Africa';
}

function regionalMealOptions(regionInput?: string | null): MealOption[] {
  const region = normalizeRegion(regionInput);
  const plans: Record<AthleteRegion, MealOption[]> = {
    Africa: [
      { name: 'Lean Kenyan-style training day', breakfast: '2–3 eggs with sukuma wiki/spinach + small sweet potato', lunch: 'grilled chicken/fish + rice or ugali + vegetables', dinner: 'beans or fish + cabbage/sukuma + small ugali', snack: 'banana + groundnuts or yoghurt', preWorkout: 'banana + 2 boiled eggs', postWorkout: 'milk + banana + eggs, or chicken/fish with rice' },
      { name: 'Githeri performance day', breakfast: 'oats or eggs + tea with little sugar', lunch: 'githeri with extra beans + avocado + vegetables', dinner: 'beef stew/chicken + sukuma + rice', snack: 'milk, fruit, or groundnuts', preWorkout: 'small githeri serving + water', postWorkout: 'chicken/beans + rice within 45–90 min' },
      { name: 'Fish recovery day', breakfast: 'eggs + avocado + fruit', lunch: 'fish + ugali + sukuma wiki', dinner: 'lentils/green grams + vegetables', snack: 'yoghurt or milk + banana', preWorkout: 'banana + yoghurt/milk', postWorkout: 'fish + rice/ugali + vegetables' },
      { name: 'High-protein local day', breakfast: 'boiled eggs + oats or sweet potato', lunch: 'beef/chicken stew + rice + cabbage', dinner: 'beans + eggs + vegetables', snack: 'groundnuts + fruit', preWorkout: 'sweet potato + eggs', postWorkout: 'milk + eggs + banana' },
      { name: 'Light fat-loss day', breakfast: 'eggs + sukuma + fruit', lunch: 'grilled chicken + salad/vegetables + controlled rice', dinner: 'fish/beans + vegetables, low oil', snack: 'avocado slice or yoghurt', preWorkout: 'banana or black tea + eggs', postWorkout: 'lean protein + vegetables + small carb portion' },
    ],
    'North America': [
      { name: 'Chicken bowl day', breakfast: 'Greek yoghurt + oats + berries', lunch: 'chicken rice bowl + beans + salad', dinner: 'salmon + potatoes + vegetables', snack: 'cottage cheese or fruit', preWorkout: 'banana + Greek yoghurt', postWorkout: 'protein shake + banana' },
      { name: 'Eggs and turkey day', breakfast: 'eggs + whole-grain toast', lunch: 'turkey wrap/bowl + salad', dinner: 'lean beef/chicken + rice + vegetables', snack: 'peanut butter toast', preWorkout: 'toast + peanut butter', postWorkout: 'turkey/chicken rice bowl' },
      { name: 'Fish recovery day', breakfast: 'protein smoothie + oats', lunch: 'tuna/salmon salad + potatoes', dinner: 'chicken/tofu + rice + vegetables', snack: 'Greek yoghurt + fruit', preWorkout: 'banana + whey/Greek yoghurt', postWorkout: 'salmon/chicken + rice' },
      { name: 'Plant-supported day', breakfast: 'oats + soy/Greek yoghurt + fruit', lunch: 'bean/tofu bowl + rice + vegetables', dinner: 'eggs/tofu/chicken + potatoes', snack: 'nuts + fruit', preWorkout: 'banana + yoghurt', postWorkout: 'tofu/chicken + rice' },
      { name: 'Lean cut day', breakfast: 'eggs + berries', lunch: 'grilled chicken salad + potatoes', dinner: 'fish + vegetables + controlled carbs', snack: 'cottage cheese', preWorkout: 'banana + Greek yoghurt', postWorkout: 'protein shake + fruit' },
    ],
    'South America': [
      { name: 'Rice and beans strength day', breakfast: 'eggs + arepa or oats + fruit', lunch: 'rice and beans + grilled chicken + avocado', dinner: 'fish + cassava/rice + vegetables', snack: 'yoghurt + fruit', preWorkout: 'banana + yoghurt', postWorkout: 'rice and beans + chicken/fish' },
      { name: 'Quinoa recovery day', breakfast: 'oats + milk/yoghurt + fruit', lunch: 'quinoa + lean beef/chicken + salad', dinner: 'beans + fish + vegetables', snack: 'nuts or milk', preWorkout: 'arepa + eggs', postWorkout: 'milk/yoghurt + fruit + eggs' },
      { name: 'Lean beef day', breakfast: 'eggs + fruit', lunch: 'lean beef + rice + vegetables', dinner: 'chicken/fish + beans + salad', snack: 'avocado + fruit', preWorkout: 'banana + yoghurt', postWorkout: 'lean beef/chicken + rice' },
      { name: 'Chicken arepa day', breakfast: 'arepa + eggs', lunch: 'chicken + beans + salad + avocado', dinner: 'fish + rice/cassava + vegetables', snack: 'milk or yoghurt', preWorkout: 'banana or small arepa', postWorkout: 'chicken + rice/beans' },
      { name: 'Fat-loss plate day', breakfast: 'eggs + fruit', lunch: 'grilled fish/chicken + vegetables + controlled rice', dinner: 'beans/lentils + salad', snack: 'yoghurt', preWorkout: 'banana + yoghurt', postWorkout: 'lean protein + small carb portion' },
    ],
    Europe: [
      { name: 'Eggs and potatoes day', breakfast: 'eggs + whole-grain toast', lunch: 'chicken/tuna + potatoes + salad', dinner: 'fish + vegetables + rice/potatoes', snack: 'Greek yoghurt + fruit', preWorkout: 'banana + yoghurt', postWorkout: 'fish/chicken + potatoes' },
      { name: 'Oats recovery day', breakfast: 'oats + Greek yoghurt + fruit', lunch: 'lentils/beans + rice + vegetables', dinner: 'lean meat/tofu + potatoes + vegetables', snack: 'cottage cheese', preWorkout: 'toast + eggs', postWorkout: 'Greek yoghurt + fruit' },
      { name: 'Mediterranean day', breakfast: 'eggs + toast + fruit', lunch: 'chicken/fish + rice + salad + olive oil', dinner: 'lentils/tofu + vegetables + potatoes', snack: 'nuts + yoghurt', preWorkout: 'banana + yoghurt', postWorkout: 'chicken/fish + rice' },
      { name: 'Lean protein day', breakfast: 'cottage cheese + fruit + oats', lunch: 'tuna/chicken salad + potatoes', dinner: 'eggs/fish + vegetables + controlled pasta/rice', snack: 'milk or yoghurt', preWorkout: 'banana + yoghurt', postWorkout: 'protein yoghurt + fruit' },
      { name: 'Muscle gain day', breakfast: 'oats + milk + yoghurt', lunch: 'chicken + pasta/rice + vegetables', dinner: 'lean beef/fish + potatoes', snack: 'nuts + milk', preWorkout: 'toast + peanut butter/yoghurt', postWorkout: 'chicken/fish + potatoes' },
    ],
    Asia: [
      { name: 'Rice and eggs day', breakfast: 'eggs + rice/congee + fruit', lunch: 'chicken/fish/tofu + rice + vegetables', dinner: 'tofu/eggs/fish + rice + soup', snack: 'soy milk + banana', preWorkout: 'banana + soy milk/yoghurt', postWorkout: 'eggs/tofu/chicken + rice' },
      { name: 'Tofu recovery day', breakfast: 'oats + yoghurt/soy milk + fruit', lunch: 'tofu/chicken rice bowl + vegetables', dinner: 'fish + rice + stir-fried vegetables', snack: 'boiled eggs or fruit', preWorkout: 'rice + eggs small portion', postWorkout: 'tofu/chicken + rice' },
      { name: 'Noodle performance day', breakfast: 'eggs + fruit', lunch: 'noodles + chicken/tofu + vegetables, light sauce', dinner: 'fish/eggs + rice + soup', snack: 'yoghurt/soy milk', preWorkout: 'banana + yoghurt/soy milk', postWorkout: 'chicken/tofu + rice' },
      { name: 'Lentil protein day', breakfast: 'oats + fruit + yoghurt', lunch: 'lentils + rice + vegetables + eggs/tofu', dinner: 'chicken/fish + vegetables + rice', snack: 'nuts + fruit', preWorkout: 'banana + soy milk', postWorkout: 'eggs/tofu + rice' },
      { name: 'Fat-loss soup day', breakfast: 'eggs + fruit', lunch: 'chicken/tofu + vegetables + controlled rice', dinner: 'fish/tofu soup + vegetables', snack: 'soy milk or yoghurt', preWorkout: 'banana if training hard', postWorkout: 'lean protein + rice/fruit' },
    ],
    Australia: [
      { name: 'Chicken rice bowl day', breakfast: 'eggs + whole-grain toast', lunch: 'chicken/tuna + rice + salad + avocado', dinner: 'lean beef/fish + potatoes + vegetables', snack: 'Greek yoghurt + fruit', preWorkout: 'banana + Greek yoghurt', postWorkout: 'protein shake + fruit' },
      { name: 'Oats and salmon day', breakfast: 'oats + Greek yoghurt + fruit', lunch: 'salmon/tuna + potatoes + salad', dinner: 'chicken/tofu + rice + vegetables', snack: 'nuts or cottage cheese', preWorkout: 'toast + peanut butter', postWorkout: 'chicken/fish + rice' },
      { name: 'Lean beef day', breakfast: 'eggs + fruit', lunch: 'lean beef + rice/potatoes + vegetables', dinner: 'fish/chicken + salad', snack: 'milk + banana', preWorkout: 'banana + yoghurt', postWorkout: 'lean beef/chicken + rice' },
      { name: 'Tofu recovery day', breakfast: 'smoothie + oats', lunch: 'tofu/chicken bowl + rice + salad', dinner: 'lentils/fish + vegetables + potatoes', snack: 'Greek yoghurt', preWorkout: 'banana + yoghurt', postWorkout: 'tofu/chicken + rice' },
      { name: 'Fat-loss day', breakfast: 'eggs + vegetables + fruit', lunch: 'chicken/tuna salad + controlled potatoes/rice', dinner: 'fish + vegetables + small carb portion', snack: 'cottage cheese or yoghurt', preWorkout: 'banana if needed', postWorkout: 'protein shake or fish/chicken + vegetables' },
    ],
  };
  // BUG-3-FIX (meal rotation): new Date().getDay() alone repeats the same meal options
  // all day and every week on the same weekday. Add the date-of-month as a secondary
  // offset so the rotation advances through the month even on the same weekday.
  const dayOfWeek   = new Date().getDay();            // 0–6
  const dateOfMonth = new Date().getDate();            // 1–31
  const rotation    = (dayOfWeek + Math.floor(dateOfMonth / 7)) % plans[region].length;
  return [...plans[region].slice(rotation), ...plans[region].slice(0, rotation)];
}

function mealOptionsText(region: string | undefined, maxOptions = 5): string {
  return regionalMealOptions(region).slice(0, maxOptions).map((m, i) =>
    `Option ${i + 1} — ${m.name}\n• Breakfast: ${m.breakfast}\n• Lunch: ${m.lunch}\n• Dinner: ${m.dinner}\n• Snack: ${m.snack}\n• Pre-workout: ${m.preWorkout}\n• Post-workout: ${m.postWorkout}`
  ).join('\n\n');
}
...
class AICoachService {
  ...
  private async buildUserContext(userId: string): Promise<UserContext> {
    ...
    if (enrollment) {
      ...
      const weeks: any[] = Array.isArray(enrollment.program.weeks) ? enrollment.program.weeks : [];

      // Strict match first
      let weekData = weeks.find((w: any) => w.weekNumber === currentWeek);
      let dayData  = weekData?.days?.find((d: any) => d.dayNumber === currentDay);

      // ROOT CAUSE FIX: if currentWeek/currentDay counters don't match any DB row
      // (common when enrollment counters drift or seeding used different numbering),
      // dayData is undefined → nextExercises = [] → generateWorkout falls to blueprint.
      // Walk all days to find the nearest non-empty training day so nextExercises is
      // always populated when an enrollment exists.
      if (!dayData?.exercises?.length) {
        const allDays = weeks.flatMap((w: any) =>
          (w.days ?? []).map((d: any) => ({ ...d, _weekNumber: w.weekNumber }))
        );
        // Prefer days at or after current position; fall back to any day with exercises
        const candidate =
          allDays.find((d: any) =>
            d.exercises?.length &&
            (d._weekNumber > currentWeek ||
             (d._weekNumber === currentWeek && d.dayNumber >= currentDay))
          ) ??
          allDays.find((d: any) => d.exercises?.length);
        if (candidate) {
          dayData  = candidate;
          weekData = weeks.find((w: any) => w.weekNumber === candidate._weekNumber) ?? weekData;
        }
      }

      const resolvedWeek = weekData?.weekNumber ?? currentWeek;
      const resolvedDay  = dayData?.dayNumber    ?? currentDay;
      ...
      program = {
        ...
        nextExercises: (dayData?.exercises ?? [])
          .map((de: any) => de.exercise?.name ?? de.exerciseName ?? '')
          .filter(Boolean),
      };

      // Program row exists but no related schedule rows matched.
      // Keep enrolled-program recommendations active using the seeded program template.
      if (!program.nextExercises.length) {
        program.nextExercises = this.getProgramTemplateExercises(
    ...
    const region   = normalizeRegion((profile as any)?.region);
  ...
  private formatContextBlock(ctx: UserContext, memory: ConversationMemory): string {
    ...
    if (ctx.program) {
      const p = ctx.program;
      lines.push('');
      lines.push(`ACTIVE PROGRAM: "${p.name}"`);
...
Never invent data. Always reference real numbers from context. "response" is always required.`;
  ...
  /**
   * Returns a deterministic program-day template when the enrolled Program exists
   * but the weeks/days/day_exercises rows are missing. This is critical for seeded
   * programs: the exported programs table may have Program rows while the relational
   * schedule tables are empty, so enrollment exists but nextExercises would otherwise
   * be [] and the coach would fall back to a generic workout.
   */
  private getProgramTemplateExercises(
    ...
    const templates: Record<string, string[][]> = {
      'beginner foundation': [
        ['Bodyweight Squats', 'Incline Push-ups', 'Glute Bridges', 'Dead Bugs', 'Low Plank'],
        ['Reverse Lunges', 'Wall Push-ups', 'Bird Dogs', 'Standing Calf Raises', 'Side Plank'],
        ['Step-ups', 'Knee Push-ups', 'Hip Hinges', 'Crunches', 'Mobility Cool-down'],
      ],
      'fat burn hiit': [
        ['Jumping Jacks', 'Bodyweight Squats', 'Mountain Climbers', 'Push-ups', 'Plank'],
        ['High Knees', 'Reverse Lunges', 'Burpees', 'Glute Bridges', 'Bicycle Crunches'],
        ['Skater Steps', 'Squat Pulses', 'Fast Feet', 'Tricep Dips', 'Side Plank'],
        ['Marching Burpees', 'Step-ups', 'Mountain Climbers', 'Superman Hold', 'Dead Bug'],
      ],
      'core power': [
        ['Dead Bugs', 'Low Plank', 'Glute Bridges', 'Bird Dogs', 'Crunches'],
        ['Side Plank', 'Reverse Crunches', 'Superman Hold', 'Heel Taps', 'Hollow Hold'],
        ['Plank Shoulder Taps', 'Bicycle Crunches', 'Leg Raises', 'Bear Hold', 'Mobility Cool-down'],
      ],
      'full body strength': [
        ['Squats', 'Push-ups', 'Hip Hinges', 'Plank', 'Reverse Lunges'],
        ['Split Squats', 'Pike Push-ups', 'Glute Bridges', 'Side Plank', 'Calf Raises'],
        ['Tempo Squats', 'Tricep Dips', 'Single-leg RDL', 'Dead Bug', 'Wall Sit'],
        ['Step-ups', 'Push-up Holds', 'Hip Thrusts', 'Bird Dogs', 'Farmer Carry'],
      ],
      'elite conditioning': [
        ['Burpees', 'Jump Squats', 'Push-ups', 'Mountain Climbers', 'Plank Jacks'],
        ['High Knees', 'Alternating Lunges', 'Bear Crawls', 'Skater Jumps', 'Side Plank'],
        ['Squat Thrusts', 'Plyo Lunges', 'Fast Push-ups', 'Bicycle Crunches', 'Sprint Intervals'],
        ['Broad Jumps', 'Push-up to Rotation', 'Jumping Lunges', 'Hollow Hold', 'Mobility Reset'],
        ['Conditioning Circuit', 'Core Finisher', 'Tempo Squats', 'Burpee Ladder', 'Cool-down Walk'],
      ],
      'athletic engine': [
        ['A-Skips', 'Squat Jumps', 'Push-ups', 'Lateral Bounds', 'Plank'],
        ['High Knees', 'Reverse Lunges', 'Bear Crawls', 'Skater Steps', 'Dead Bugs'],
        ['Fast Feet', 'Broad Jumps', 'Mountain Climbers', 'Side Plank', 'Sprint Intervals'],
        ['Agility Shuffle', 'Tempo Squats', 'Push-up to Rotation', 'Burpees', 'Mobility Reset'],
      ],
      'lean muscle builder': [
        ['Tempo Squats', 'Push-ups', 'Single-leg RDL', 'Plank', 'Calf Raises'],
        ['Split Squats', 'Pike Push-ups', 'Glute Bridges', 'Side Plank', 'Dead Bugs'],
        ['Bulgarian Split Squats', 'Tricep Dips', 'Hip Thrusts', 'Superman Hold', 'Wall Sit'],
        ['Step-ups', 'Push-up Eccentrics', 'Reverse Lunges', 'Hollow Hold', 'Mobility Cool-down'],
        ['Full-body Hypertrophy Circuit', 'Squat Pulses', 'Push-up Ladder', 'Glute Bridge Hold', 'Core Finisher'],
      ],
      '30-minute shred': [
        ['Bodyweight Squats', 'Push-ups', 'Mountain Climbers', 'Reverse Lunges', 'Plank'],
        ['Jumping Jacks', 'Glute Bridges', 'High Knees', 'Tricep Dips', 'Bicycle Crunches'],
        ['Step-ups', 'Squat Thrusts', 'Skater Steps', 'Superman Hold', 'Dead Bugs'],
        ['Burpees', 'Tempo Squats', 'Push-up to Rotation', 'Side Plank', 'Cool-down Walk'],
        ['Conditioning Circuit', 'Lower-body Burnout', 'Core Finisher', 'Mobility Reset', 'Breathing Drill'],
      ],
      'joint strong reset': [
        ['Chair Squats', 'Wall Push-ups', 'Glute Bridges', 'Bird Dogs', 'Hip Mobility'],
        ['Step-ups', 'Scapular Wall Slides', 'Dead Bugs', 'Calf Raises', 'Thoracic Rotations'],
        ['Supported Lunges', 'Incline Push-ups', 'Hip Hinges', 'Side Plank', 'Mobility Flow'],
      ],
      'elite core athlete': [
        ['Hollow Hold', 'Side Plank', 'Dead Bug', 'Bear Crawl', 'Glute Bridge March'],
        ['Plank Shoulder Taps', 'Reverse Crunches', 'Bird Dog Rows', 'Superman Hold', 'Hip Mobility'],
        ['Hollow Rocks', 'Copenhagen Side Plank', 'Mountain Climbers', 'Anti-rotation Hold', 'Back Extension'],
        ['Bear Plank', 'Leg Raises', 'Side Plank Hip Dips', 'Plank Walkouts', 'Breathing Reset'],
      ],
      'home hybrid performance': [
        ['Squats', 'Push-ups', 'Mountain Climbers', 'Glute Bridges', 'Plank'],
        ['Reverse Lunges', 'Pike Push-ups', 'High Knees', 'Dead Bugs', 'Mobility Flow'],
        ['Step-ups', 'Tricep Dips', 'Burpees', 'Side Plank', 'Hip Hinges'],
        ['Tempo Squats', 'Push-up to Rotation', 'Skater Steps', 'Superman Hold', 'Cool-down Walk'],
        ['Hybrid Conditioning Circuit', 'Core Finisher', 'Lower-body Strength', 'Upper-body Strength', 'Recovery Mobility'],
      ],
      'metabolic torch': [
        ['Push-ups', 'Tricep Dips', 'Russian Twists', 'Plank', 'High Knees'],
        ['Squats', 'Reverse Lunges', 'Glute Bridges', 'Mountain Climbers', 'Side Plank'],
      ],
      'max force': [
        ['Push-ups', 'Squats', 'Lunges', 'Plank', 'Hip Hinges'],
        ['Tempo Squats', 'Pike Push-ups', 'Glute Bridges', 'Dead Bug', 'Wall Sit'],
      ],
      'iron flow': [
        ['Squats', 'Lunges', 'Glute Bridges', 'Crunches', 'Calf Raises'],
        ['Split Squats', 'Hip Hinges', 'Step-ups', 'Dead Bugs', 'Side Plank'],
      ],
    };

    const matchedKey = Object.keys(templates).find(k => name.includes(k));
    let pool = matchedKey ? templates[matchedKey] : undefined;

    if (!pool) {
      if (cat.includes('hiit') || cat.includes('conditioning') || name.includes('shred')) {
        pool = templates['fat burn hiit'];
      } else if (cat.includes('core')) {
        pool = templates['core power'];
      } else if (cat.includes('mobility') || name.includes('joint')) {
        pool = templates['joint strong reset'];
      } else if (cat.includes('strength') || level.includes('advanced')) {
        pool = templates['full body strength'];
      } else {
        pool = templates['home hybrid performance'];
      }
    }

    const idx = Math.abs(((currentWeek - 1) * Math.max(pool.length, 1)) + (currentDay - 1)) % pool.length;
    return pool[idx];
  }
  ...
  private getGoalWorkoutBlueprint(ctx: UserContext): { title: string; focus: string; exercises: { name: string; sets: number; reps: string; note?: string }[] } {
    ...
    // BUG-3-FIX: Use a multi-signal rotation index instead of a static single workout.
    // Combines day-of-week + recent log count + hour-of-day to produce a stable but
    // varied index that changes each session even on the same day of the week.
    // This prevents the fallback from always returning the same workout.
    const completedLogs = ctx.recentLogs.filter(l => !l.skipped);
    const recentDone = completedLogs.length;
    const recentSignature = completedLogs
      .slice(0, 5)
      .map(l => `${l.date}:${l.exercise}:${l.category}`)
      .join('|');
    const signatureHash = Array.from(recentSignature).reduce((sum, ch) => sum + ch.charCodeAt(0), 0);
    const rotationBase = new Date().getDay();          // 0–6: changes daily
    const hourBucket = Math.floor(new Date().getHours() / 6); // changes across morning/afternoon/evening/night
    const sessionOffset = (recentDone + signatureHash + hourBucket) % 3;
    const rotationIdx = (rotationBase + sessionOffset) % 3;  // final 0/1/2 index

    if (goal.includes('loss') || goal.includes('cut') || goal.includes('fat')) {
      // BUG-3-FIX: 3 fat-loss rotations instead of 1 static session.
      const fatLossVariants = [
        {
        ...
        {
          title: 'Fat-Loss Lower + Cardio Burst',
          focus: 'leg drive and metabolic conditioning',
          exercises: [
            { name: 'Jump Squats', sets: 4, reps: '12', note: 'land soft' },
            { name: 'Glute Bridge', sets: 4, reps: '15', note: '2 s hold at top' },
            { name: 'Step-ups', sets: 3, reps: '10 each leg' },
            { name: 'Burpees', sets: 3, reps: '8', note: 'RPE 8' },
            { name: 'Side Plank', sets: 3, reps: '30 s each side' },
          ],
        },
        {
          title: 'Fat-Loss Upper + Core Finisher',
          focus: 'upper body strength with core stability for calorie burn',
          exercises: [
            { name: 'Push-ups', sets: 4, reps: level === 'beginner' ? '8' : '15', note: 'controlled descent' },
            { name: 'Tricep Dips (chair)', sets: 3, reps: '12' },
            { name: 'Superman Hold', sets: 3, reps: '30 s' },
            { name: 'High Knees', sets: 4, reps: '40 s', note: 'RPE 8' },
            { name: 'Dead Bug', sets: 3, reps: '10 each side', note: 'slow and controlled' },
          ],
        },
      ];
      return fatLossVariants[rotationIdx];
    }

    if (goal.includes('muscle') || goal.includes('gain') || goal.includes('bulk') || goal.includes('strength')) {
      // BUG-3-FIX: 3 muscle/strength rotations (lower, upper, push-pull) instead of 2.
      // Previously both trainedLower=true AND trainedUpper=true → always "upper",
      // so users who trained consistently always got the same session.
      const muscleVariants = [
        {
          title: 'Muscle-Building Lower Body + Core',
          focus: 'progressive overload on compound leg movements',
          exercises: [
        ...
        },
        {
          ...
          focus: 'chest, back, and shoulder hypertrophy',
          exercises: [
        ...
        {
          title: 'Muscle-Building Push + Pull Full Body',
          focus: 'balanced push-pull split for total upper and lower development',
          exercises: [
            { name: 'Push-ups or Chest Press', sets: 4, reps: '10', note: 'full range' },
            { name: 'Bodyweight Row or Dumbbell Row', sets: 4, reps: '10' },
            { name: 'Goblet Squat', sets: 3, reps: '10', note: 'pause at bottom' },
            { name: 'Hip Thrust or Glute Bridge', sets: 3, reps: '12', note: 'squeeze at top' },
            { name: 'Plank Hold', sets: 3, reps: '45 s' },
          ],
        },
      ];
      // Respect recent history — if only lower trained, prefer upper next; if both, use rotation
      let variantIdx = rotationIdx;
      if (!trainedLowerRecently && trainedUpperRecently) variantIdx = 0; // force lower
      else if (trainedLowerRecently && !trainedUpperRecently) variantIdx = 1; // force upper
      // both trained or neither → rotationIdx (0/1/2 cycling)
      return muscleVariants[variantIdx];
    }

    // BUG-3-FIX: 3 maintenance/general fitness rotations instead of 1 static session.
    const maintenanceVariants = [
      {
        title: 'Balanced Full Body — Strength Focus',
      ...
      {
        title: 'Balanced Full Body — Endurance Focus',
        focus: 'muscular endurance and cardiovascular efficiency',
        exercises: [
          { name: 'Jumping Jacks', sets: 1, reps: '3 min warm-up' },
          { name: 'Bodyweight Squat', sets: 3, reps: '15–20', note: 'controlled pace' },
          { name: 'Push-up to T-rotation', sets: 3, reps: '8 each side' },
          { name: 'Reverse Lunge', sets: 3, reps: '12 each leg' },
          { name: 'Bear Crawl', sets: 3, reps: '20 m or 30 s' },
        ],
      },
      {
        title: 'Balanced Full Body — Mobility + Strength',
        focus: 'joint health, flexibility, and functional movement',
        exercises: [
          { name: 'Cat-Cow + Hip Circles', sets: 1, reps: '5 min mobility warm-up' },
          { name: 'Deep Goblet Squat Hold', sets: 3, reps: '10 + 30 s hold', note: 'open hips' },
          { name: 'Push-up', sets: 3, reps: '10', note: 'slow 3-1-2 tempo' },
          { name: 'Single-Leg RDL', sets: 3, reps: '8 each leg', note: 'balance focus' },
          { name: 'Side Plank + Hip Dip', sets: 3, reps: '8 each side' },
        ],
      },
    ];
    return maintenanceVariants[rotationIdx];
  }
  ...
  private workoutFuelPlan(ctx: UserContext, workoutTitle = "today's workout"): string {
    ...
    const meal = regionalMealOptions(ctx.region)[0];

    return `Fuel for ${workoutTitle}:
• Pre-workout: ${meal.preWorkout}
• Post-workout: ${meal.postWorkout}
• Protein target today: ${protein}g`;
  }

  private generateWorkout(ctx: UserContext): CoachResponse {
    const blueprint = this.getGoalWorkoutBlueprint(ctx);

    // BUG-1-FIX: When the user is enrolled in a program, lead the workout with their
    // scheduled program exercises instead of the generic goal blueprint. The old code
    // acknowledged the program in a "programNote" comment but still showed a generic
    // blueprint workout — meaning enrolled users never saw their actual program exercises
    // when the LLM picked generate_workout.
    // Strategy: program exercises lead the session; blueprint fills remaining slots.
    if (ctx.program?.nextExercises?.length) {
      const prog = ctx.program;
      const defaultSets = blueprint.exercises[0]?.sets ?? 3;
      const defaultReps = blueprint.exercises[0]?.reps ?? '10–12';

      const progLines = prog.nextExercises.map((name, i) => {
        const match = blueprint.exercises.find(e =>
          e.name.toLowerCase().includes(name.toLowerCase().split(' ')[0]) ||
          name.toLowerCase().includes(e.name.toLowerCase().split(' ')[0])
        );
        const s = match?.sets ?? defaultSets;
        const r = match?.reps ?? defaultReps;
        const note = i === 0 ? 'compound first' : (match?.note ?? undefined);
        return `• ${name} — ${s}×${r}${note ? ` (${note})` : ''}`;
      });

      const covered = new Set(prog.nextExercises.map(n => n.toLowerCase()));
      const extras = blueprint.exercises
        .filter(e => !covered.has(e.name.toLowerCase()))
        .slice(0, Math.max(0, 5 - prog.nextExercises.length))
        .map(e => `• ${e.name} — ${e.sets}×${e.reps}${e.note ? ` (${e.note})` : ''}`);

      const allLines = [...progLines, ...extras].join('\n');

      const volNote = ctx.volumeChangePct < -10
        ? `\n\nVolume is down ${Math.abs(ctx.volumeChangePct)}% — complete every set before adding load.`
        : ctx.volumeChangePct > 15
        ? `\n\nVolume is up ${ctx.volumeChangePct}% — keep RPE 7–8 to avoid overreaching.`
        : '';
      ...
      return {
        success: true,
        reply,
        data: { workout: blueprint, source: 'program_primary_blueprint_secondary', program: prog },
      };
    }

    // No active program — pure goal/history/level blueprint
    const exerciseLines = blueprint.exercises.map(e =>
      `• ${e.name} — ${e.sets}×${e.reps}${e.note ? ` (${e.note})` : ''}`
    ).join('\n');

    const historyNote = ctx.recentLogs.length
      ? `\n\nBased on your recent logs, this fills the most useful training gap for your ${ctx.fitnessGoal} goal.`
      : `\n\nBuilt from your goal, level, and current profile.`;
    ...
    const reply = `${blueprint.title}\n\nFocus: ${blueprint.focus}\n\nWorkout:\n${exerciseLines}${historyNote}${volNote}\n\n${this.workoutFuelPlan(ctx, blueprint.title)}\n\nWarm up 5–7 min. Rest 60–90 s. Tell me when you finish and I can log it.`;

    return {
      success: true,
      reply,
      data: { workout: blueprint, source: 'history_goal_profile' },
    };
  }

    private async nextWorkout(userId: string, ctx: UserContext): Promise<CoachResponse> {
    const programCtx = ctx.program;

    const enrollment: any = await prisma.programEnrollment.findFirst({
      where: { userId, isActive: true },
      include: {
        program: {
          include: {
    ...
    if (!programCtx) {
      return this.generateWorkout(ctx);
    }
    ...
    const weeks = Array.isArray(enrollment.program.weeks) ? enrollment.program.weeks : [];
    let selectedWeek = weeks.find((w: any) => w.weekNumber === programCtx.currentWeek) ?? weeks[0];
    let selectedDay = selectedWeek?.days?.find((d: any) => d.dayNumber === programCtx.currentDay) ?? selectedWeek?.days?.[0];

    // If current day is empty because counters drifted, use the nearest saved training day.
    if (!selectedDay?.exercises?.length) {
      const allDays = weeks.flatMap((w: any) => (w.days ?? []).map((d: any) => ({ ...d, weekNumber: w.weekNumber })));
      selectedDay = allDays.find((d: any) => d.exercises?.length && d.weekNumber >= programCtx.currentWeek && d.dayNumber >= programCtx.currentDay)
        ?? allDays.find((d: any) => d.exercises?.length)
        ?? selectedDay;
      selectedWeek = selectedDay?.weekNumber ? weeks.find((w: any) => w.weekNumber === selectedDay.weekNumber) ?? selectedWeek : selectedWeek;
    }
    ...
    if (!dayEx.length && programCtx.nextExercises.length) {
      ...
      return {
        success: true,
        reply,
        data: { exercises: programCtx.nextExercises.map(name => ({ name })), week: programCtx.currentWeek, day: programCtx.currentDay, source: 'program_template_no_day_rows' },
      };
    }
    ...
    const shownWeek = selectedWeek?.weekNumber ?? programCtx.currentWeek;
    const shownDay = selectedDay?.dayNumber ?? programCtx.currentDay;
    ...
    return {
      success: true,
      reply,
      data: { exercises: dayEx.map((de: any) => de.exercise ?? { name: de.exerciseName }).filter(Boolean), week: shownWeek, day: shownDay },
    };
  }
  ...
  private async weeklyProgram(userId: string, ctx: UserContext): Promise<CoachResponse> {
            ...
            weeks: {
              include: {
                days: {
                  include: {
                    exercises: {
                      include: { exercise: { select: { name: true, category: true, description: true } } },
                      orderBy: { orderIndex: 'asc' },
                    },
                  },
                  orderBy: { dayNumber: 'asc' },
    ...
    const weeks = Array.isArray(enrollment?.program?.weeks) ? enrollment.program.weeks : [];
    let selectedWeek = weeks.find((w: any) => w.weekNumber === programCtx.currentWeek) ?? weeks[0];
    if (!selectedWeek?.days?.length) {
      selectedWeek = weeks.find((w: any) => w.days?.length) ?? selectedWeek;
    }
    ...
    const scheduleLines = days.map((d: any) => {
      const dayLabel = d.isRestDay
        ? 'Rest Day'
        : (d.exercises ?? []).map((de: any) => de.exercise?.name ?? de.exerciseName ?? '?').join(' · ');
      const marker = d.dayNumber === programCtx.currentDay ? ' ← TODAY' : '';
      const dayName = d.name ? ` — ${d.name}` : '';
      return `Day ${d.dayNumber}${dayName}${marker}: ${dayLabel || 'Mobility / recovery'}`;
    }).join('\n');
  ...
  private nutritionPlan(ctx: UserContext): CoachResponse {
    ...
    const mealLines = mealOptionsText(ctx.region, 5);
    ...
    return {
      ...
      reply: `Personalised Nutrition Plan — ${ctx.fitnessGoal.toUpperCase()}\n\n` +
        ...
        mealLines + '\n\n' +
        `Key Rules:\n${tips}`,
      data: { targets: { calories: cal, protein: pro, carbs: carb, fat }, meals: regionalMealOptions(ctx.region).slice(0, 5), predictedWorkout,  },
    };
  }
    ...
    if (/what.*eat.*(today|now)|today.*eat|eat.*today/.test(question) || question.trim() === '') {
      ...
      return {
        ...
        reply: `Today's eating guide — ${ctx.fitnessGoal.toUpperCase()}\n\n` +
          ...
          (mealsLogged > 0
            ? `Logged so far: ${mealsLogged} meal${mealsLogged !== 1 ? 's' : ''}. Remaining estimate: ${Math.round(remaining)} kcal and ${Math.round(proteinLeft)}g protein.\n\n`
            : `Suggested meals for today:\n\n`) +
          `${mealOptionsText(ctx.region, 5)}\n\n` +
          `Use these as rotating options. Keep the main protein/carb that fits your workout, but switch the full meal combination through the week.`,
      };
    }
  ...
  private async nutritionSummary(userId: string, ctx: UserContext): Promise<CoachResponse> {
    ...
    try {
      const nutritionLog = (prisma as any).nutritionLog;
      if (!nutritionLog) {
        return { success: true, reply: `Today's meal suggestions\n\nTargets: ${ctx.dailyCalorieTarget} kcal | ${ctx.dailyProteinTarget}g protein\n\n${mealOptionsText(ctx.region, 5)}` };
      }
...
` +
          `Rotate these options through the week instead of repeating the same meals every day.`,
      };
    }
  ...
  private async saveMemory(userId: string, memory: ConversationMemory): Promise<void> {
    // Guard: never attempt upsert with a falsy userId — it causes a Prisma P2011
    // null constraint violation on the user_id / userId column.
    if (!userId) {
      console.error('[AICoach] saveMemory called with empty userId — skipping');
      return;
    }

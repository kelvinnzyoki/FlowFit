...
export async function getCurrentSubscription(
  ...
  try {
    ...
    return {
      ...
      hasPaystackSubscription:  !!sub.paystackSubscriptionCode && !!sub.paystackEmailToken,
      isPendingActivation,
    } as any;
...
export async function handleMpesaSuccess(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // FIX-2: Validate received amount against plan price to prevent under-payment activation.
    const expectedKes =
    ...
    if (typeof expectedKes !== 'number' || expectedKes <= 0) {
      throw new Error(
        `[handleMpesaSuccess] Expected KES amount is not configured for subscription ${mpesaTx.subscriptionId}`
      );
    }

    if (typeof amountKes !== 'number' || amountKes <= 0 || amountKes < expectedKes) {
      throw new Error(
        `[handleMpesaSuccess] Amount mismatch: received KES ${amountKes ?? 'missing'}, ` +
...
export async function reactivateSubscription(
  ...
  if (hasPaystackSub) {
    ...
    if (needsNewPaystackSub) {
      ...
      const customer = await fetchPaystackCustomer(sub.paystackCustomerCode);
      const hasReusableCard = Array.isArray(customer.authorizations) &&
        customer.authorizations.some((auth: any) =>
          auth?.authorization_code &&
          auth?.reusable === true &&
          String(auth?.channel ?? '').toLowerCase() === 'card'
        );

      if (!hasReusableCard) {
        throw new Error(
          'Cannot reactivate: this Paystack customer has no reusable card authorization. Please start a new checkout.',
        );
      }
      ...
      const newPaystackSub = await createPaystackSubscription(
        ...
        planCode,
        undefined,          // Paystack uses the customer's most recent card authorization
        sub.currentPeriodEnd, // First charge happens on the original renewal date — no double-bill
      );

...
export async function cancelSubscription(
  ...
  if (sub.provider === 'PAYSTACK') {
    ...
    // If codes are not available yet (INCOMPLETE/pending webhook), fall back to DB-only cancel.
    if (hasPaystackCodes) {
...
export async function upgradeSubscription(
  ...
  const amountKes = newInterval === 'YEARLY'
    ? (newPlan as any).mpesaYearlyKes
    : (newPlan as any).mpesaMonthlyKes;
  const amountKobo = amountKes ? amountKes * 100 : 0;

  if (!paystackPlanCode) {
    throw new Error(`Paystack plan code not configured for ${newPlan.slug}/${newInterval}`);
  }
  if (!amountKobo || amountKobo <= 0) {
    throw new Error(`Paystack KES amount not configured for ${newPlan.slug}/${newInterval}`);
  }
  ...
  await logEvent(sub.id, 'UPGRADED', prevStatus, 'CANCELLED',
    {
  ...
  const callbackUrl = `${process.env.FRONTEND_URL ?? ''}/subscription?action=upgrade`;
...
// ─── Billing portal ───────────────────────────────────────────────────────────
/**
 * Paystack's supported billing-management flow is generated from the
 * subscription_code. The local DB must also have email_token because cancel and
 * reactivate require code + token; this keeps the ACTIVE PAYSTACK invariant strict.
 */
export async function getBillingPortalUrl(userId: string): Promise<string> {
  ...
  if (!sub?.paystackSubscriptionCode || !sub?.paystackEmailToken) {
    throw new Error('No complete Paystack subscription management codes found');
  }

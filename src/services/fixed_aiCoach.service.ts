...
function regionalMealOptions(regionInput?: string | null): MealOption[] {
  ...
  const rotation = new Date().getDay() % plans[region].length;
...
type NormalizedProgramExercise = {
  name: string;
  category?: string;
  description?: string;
  sets?: number | null;
...
type NormalizedProgramDay = {
  ...
  dayNumber: number;
  title?: string;
  exercises: NormalizedProgramExercise[];
};
...
function normalizeProgramExercise(raw: any): NormalizedProgramExercise | null {
  const source = raw?.exercise && typeof raw.exercise === 'object' ? raw.exercise : raw;
  const name = source?.name ?? raw?.exerciseName ?? raw?.name ?? raw?.title;
  if (!name || String(name).trim().toLowerCase() === 'exercise') return null;
  return {
    name: String(name).trim(),
    category: source?.category ?? raw?.category ?? 'General',
    description: source?.description ?? raw?.description ?? undefined,
    sets: raw?.sets ?? source?.sets ?? null,
    reps: raw?.reps ?? source?.reps ?? null,
    restSeconds: raw?.restSeconds ?? raw?.rest_seconds ?? raw?.rest ?? null,
    notes: raw?.notes ?? source?.notes ?? undefined,
  };
}

function normalizeProgramDays(program: any): NormalizedProgramDay[] {
  const out: NormalizedProgramDay[] = [];

  const pushDay = (weekNumber: number, day: any, fallbackDay: number) => {
    const exercises = Array.isArray(day?.exercises)
      ? day.exercises.map(normalizeProgramExercise).filter(Boolean) as NormalizedProgramExercise[]
      : [];
    if (!exercises.length) return;
    out.push({
      weekNumber,
      dayNumber: Number(day?.dayNumber ?? day?.day_number ?? fallbackDay),
      title: day?.title ?? day?.name ?? undefined,
      exercises,
    });
  };

  // Prisma relation shape: program.weeks[].days[].exercises[]
  if (Array.isArray(program?.weeks)) {
    program.weeks.forEach((week: any, wi: number) => {
      const weekNumber = Number(week?.weekNumber ?? week?.week_number ?? wi + 1);
      if (Array.isArray(week?.days)) week.days.forEach((day: any, di: number) => pushDay(weekNumber, day, di + 1));
    });
  }

  // Some seeded/catalogue programs store the full workout plan in Program.metadata instead of DayExercise rows.
  const metadata = typeof program?.metadata === 'string' ? safeJsonParse(program.metadata) : program?.metadata;
  const meta = asObject(metadata);
  const candidates = [meta, asObject(meta?.program), asObject(meta?.plan), asObject(meta?.template)].filter(Boolean) as Record<string, any>[];

  for (const candidate of candidates) {
    if (Array.isArray(candidate.weeks)) {
      candidate.weeks.forEach((week: any, wi: number) => {
        const weekNumber = Number(week?.weekNumber ?? week?.week_number ?? wi + 1);
        if (Array.isArray(week?.days)) week.days.forEach((day: any, di: number) => pushDay(weekNumber, day, di + 1));
      });
    }
    if (Array.isArray(candidate.days)) {
      candidate.days.forEach((day: any, di: number) => pushDay(Number(day?.weekNumber ?? day?.week_number ?? 1), day, di + 1));
    }
    if (Array.isArray(candidate.schedule)) {
      candidate.schedule.forEach((day: any, di: number) => pushDay(Number(day?.weekNumber ?? day?.week_number ?? 1), day, di + 1));
    }
  }

  const seen = new Set<string>();
  return out.filter(day => {
    const key = `${day.weekNumber}:${day.dayNumber}:${day.exercises.map(e => e.name).join('|')}`;
...
function selectProgramDay(program: any, currentWeek = 1, currentDay = 1): NormalizedProgramDay | null {
  const days = normalizeProgramDays(program);
  if (!days.length) return null;
  return days.find(d => d.weekNumber === currentWeek && d.dayNumber === currentDay)
      ?? days.find(d => d.weekNumber === currentWeek && d.exercises.length)
      ?? days.find(d => d.exercises.length)
      ?? null;
}
...
class AICoachService {
  ...
  private async buildUserContext(userId: string): Promise<UserContext> {
    ...
    const [
      profile, subscription, activeEnrollments,
    ...
    // Program — prefer the active enrollment whose current day can actually resolve exercises.
    // Supports both relational Week/Day/DayExercise rows and catalogue programs stored in Program.metadata.
    const enrollment = (activeEnrollments ?? []).find(e =>
      Boolean(selectProgramDay(e.program, e.currentWeek, e.currentDay))
    ) ?? (activeEnrollments ?? [])[0];

    let program: UserContext['program'] | undefined;
    if (enrollment) {
      const { currentWeek, currentDay, completedDays } = enrollment;
      const normalizedDays = normalizeProgramDays(enrollment.program);
      const selectedDay = selectProgramDay(enrollment.program, currentWeek, currentDay);
      const totalDays = Math.max(
        enrollment.program.durationWeeks * enrollment.program.daysPerWeek,
      ...
      program = {
        ...
        nextExercises: selectedDay?.exercises.map(e => e.name).filter(Boolean) ?? [],
      };
    }
...
- Suggest practical, familiar foods, but do not mention the athlete's region unless they ask.
...
Never invent data. Always reference real numbers from context. "response" is always required.`;
  ...
  private getGoalWorkoutBlueprint(ctx: UserContext): { title: string; focus: string; exercises: { name: string; sets: number; reps: string; note?: string }[] } {
    ...
    const recentNames = new Set(ctx.recentLogs.filter(l => !l.skipped).slice(0, 8).map(l => l.exercise.toLowerCase()));

    const chooseLeastRecent = <T extends { title: string; exercises: { name: string }[] }>(plans: T[]): T => {
      return [...plans].sort((a, b) => {
        const aHits = a.exercises.filter(e => recentNames.has(e.name.toLowerCase())).length;
        const bHits = b.exercises.filter(e => recentNames.has(e.name.toLowerCase())).length;
        return aHits - bHits;
      })[0];
    };
    ...
    const fatLoss = [
      ...
      {
        title: 'Low-Impact Fat Burn Circuit',
        focus: 'steady calorie burn with joint-friendly conditioning',
        exercises: [
          { name: 'Step-ups', sets: 4, reps: '12 each leg' },
          { name: 'Incline Push-ups', sets: 3, reps: '10–15' },
          { name: 'Glute Bridges', sets: 4, reps: '15' },
          { name: 'Fast March in Place', sets: 5, reps: '45 s' },
          { name: 'Side Plank', sets: 3, reps: '25 s each side' },
        ],
      },
      {
        title: 'Metabolic Core + Legs',
        focus: 'legs and core with high breathing demand',
        exercises: [
          { name: 'Jumping Jacks', sets: 4, reps: '40 s' },
          { name: 'Walking Lunges', sets: 3, reps: '12 each leg' },
          { name: 'Squat Pulses', sets: 3, reps: '20' },
          { name: 'High Knees', sets: 4, reps: '30 s' },
    ...
    const muscle = [
      ...
      },
      {
        ...
        focus: 'push-pull strength with clean reps',
        exercises: [
      ...
      },
      {
        ...
        focus: 'glutes, hamstrings, back, and trunk strength',
        exercises: [
          { name: 'Hip Thrusts', sets: 4, reps: '10–12' },
          { name: 'Single-leg Romanian Deadlift', sets: 3, reps: '8 each leg' },
          { name: 'Chair Rows or Dumbbell Rows', sets: 4, reps: '10–12' },
          { name: 'Hamstring Walkouts', sets: 3, reps: '8–10' },
          { name: 'Hollow Hold', sets: 3, reps: '25–35 s' },
        ],
      },
    ];
    ...
    const maintenance = [
      ...
      {
        title: 'Mobility + Strength Maintenance',
        focus: 'move well, maintain strength, recover better',
        exercises: [
          { name: 'World’s Greatest Stretch', sets: 2, reps: '5 each side' },
          { name: 'Split Squat', sets: 3, reps: '10 each leg' },
          { name: 'Pike Push-up', sets: 3, reps: '8–10' },
          { name: 'Bird Dog', sets: 3, reps: '10 each side' },
          { name: 'Farmer Carry or Loaded Carry', sets: 4, reps: '30–45 s' },
        ],
      },
    ];

    if (goal.includes('loss') || goal.includes('cut') || goal.includes('fat')) return chooseLeastRecent(fatLoss);
    if (goal.includes('muscle') || goal.includes('gain') || goal.includes('bulk') || goal.includes('strength')) return chooseLeastRecent(muscle);
    return chooseLeastRecent(maintenance);
  }
...
• Protein target today: ${protein}g`;
  ...
  private async nextWorkout(userId: string, ctx: UserContext): Promise<CoachResponse> {
    ...
    const enrollment =
      activeEnrollments.find(e => ctx.program?.name && e.program.name === ctx.program.name && selectProgramDay(e.program, e.currentWeek, e.currentDay)) ??
      activeEnrollments.find(e => selectProgramDay(e.program, e.currentWeek, e.currentDay)) ??
      activeEnrollments[0];

    const selectedDay = enrollment ? selectProgramDay(enrollment.program, enrollment.currentWeek, enrollment.currentDay) : null;

    if (!selectedDay?.exercises.length) {
      return this.generateWorkout(ctx);
    }

    const exercisesList = selectedDay.exercises.map((ex, i) => {
      const meta = [
        ex.category || 'General',
      ...
      return `${i + 1}. ${ex.name}${meta ? ` [${meta}]` : ''}${desc}`;
    }).join('\n');

    const fatNote = (ctx.fatigueScore ?? 0) >= 6
      ? `\n\nFatigue is elevated — keep RPE around 6–7 and prioritise clean form.`
      : '';

    const programName = enrollment?.program.name ?? ctx.program?.name ?? 'Your program';
    const reply = `${programName} — Week ${selectedDay.weekNumber}, Day ${selectedDay.dayNumber}${selectedDay.title ? `: ${selectedDay.title}` : ''}\n\n` +
                  `${exercisesList}\n\n` +
                  `Warm up 5–7 minutes. Rest 60–90 seconds between sets.${fatNote}\n\n` +
                  `${this.workoutFuelPlan(ctx, `${programName} Week ${selectedDay.weekNumber} Day ${selectedDay.dayNumber}`)}\n\n` +
                  `Tell me when you're done and I'll log it for you.`;

    return {
      success: true,
      reply,
      data: { exercises: selectedDay.exercises, week: selectedDay.weekNumber, day: selectedDay.dayNumber, source: 'active_program' },
    };
  }
...
${this.workoutFuelPlan(ctx, base.title)}`,
  ...
  const enrollment =
    activeEnrollments.find(e => ctx.program?.name && e.program.name === ctx.program.name && normalizeProgramDays(e.program).length) ??
    activeEnrollments.find(e => normalizeProgramDays(e.program).length) ??
    activeEnrollments[0];

  const allDays = enrollment ? normalizeProgramDays(enrollment.program) : [];
  const weekNumber = enrollment?.currentWeek ?? ctx.program.currentWeek;
  const days = allDays.filter(d => d.weekNumber === weekNumber).length
    ? allDays.filter(d => d.weekNumber === weekNumber)
    : allDays.slice(0, Math.max(ctx.program.daysPerWeek, 1));

  if (!days.length) {
    const base = this.getGoalWorkoutBlueprint(ctx);
    return {
      success: true,
      reply: `Week Plan:\n• Day 1: ${base.title}\n• Day 2: Mobility + 25 min walk\n• Day 3: Repeat main lifts with 1 extra rep per set if form is solid\n• Day 4: Rest or light stretching\n• Day 5: Full-body conditioning\n\n${this.workoutFuelPlan(ctx, base.title)}`,
      data: { source: 'history_goal_profile', workout: base },
    };
  }

  const scheduleLines = days.map(d => {
    const dayLabel = d.exercises.slice(0, 4).map(ex => ex.name).join(' · ');
    const dayName = d.title ? `: ${d.title}` : '';
    const marker = d.dayNumber === (enrollment?.currentDay ?? ctx.program.currentDay) ? ' ← current' : '';
    return `Day ${d.dayNumber}${dayName}${marker}: ${dayLabel}`;
  }).join('\n');

  const programName = enrollment?.program.name ?? ctx.program.name;
  const reply = `${programName} — Week ${weekNumber}\n\n${scheduleLines}\n\n${ctx.program.completedDays}/${ctx.program.totalDays} sessions complete · ${ctx.program.pctComplete}% done`;

  return { success: true, reply, data: { days, source: 'active_program' } };
  }
  ...
  private async nutritionSummary(userId: string, ctx: UserContext): Promise<CoachResponse> {
    ...
    if (!logs.length) {
      ...
      return {
        success: true,
        reply: `Today's meal suggestions\n\n` +

...
export async function getBillingPortalUrl(userId: string): Promise<string> {
  ...
  if ((!subscriptionCode || !emailToken) && sub.paystackCustomerCode) {
    ...
    if (codeResult.ok) {
      subscriptionCode = codeResult.subscriptionCode ?? subscriptionCode;
      emailToken       = codeResult.emailToken       ?? emailToken;
    }
  }

  if (subscriptionCode && !emailToken) {
    try {
      const psSub = await fetchPaystackSubscription(subscriptionCode);
      emailToken = psSub.email_token || emailToken;
      if (emailToken) {
        await prisma.subscription.update({
          where: { id: sub.id },
          data:  { paystackEmailToken: emailToken },
      ...
      console.warn('[getBillingPortalUrl] Could not fetch Paystack subscription:', err);
    }
  }

  if (!emailToken) {
    throw new Error(
      'Paystack billing portal is not available because paystackEmailToken is missing. ' +
      'The subscription should not be ACTIVE until Paystack returns subscription_code and email_token.',
    );
  }

  return `https://paystack.com/manage-subscription/${encodeURIComponent(emailToken)}`;
}

 ...
 *
 *   This ordering guarantees a Subscription record always exists before money
 *   is requested, enabling idempotent retries. The MpesaTransaction is created
 *   after the STK push so Daraja IDs are present at insert time — satisfying
 *   the schema's non-null constraints without any migration.
 *   The previous ordering (STK first, DB second) allowed the user's phone to
 *   be prompted while the DB write subsequently failed, producing a payment
 *   the webhook could never activate.
 */
export async function initMpesaPayment(
  ...
  // ── Step 2: Create the Subscription BEFORE firing the STK push ───────────
  // The Subscription row is written first so an idempotency check can reuse
  // it on double-taps. No MpesaTransaction is created yet because the schema
  // requires checkoutRequestId and merchantRequestId to be non-null — those
  // IDs only exist after Daraja responds. Creating the Subscription here
  // ensures something exists in the DB before any money is requested.

  let subscription: { id: string };

  try {
    // Reuse an existing INCOMPLETE subscription when the user taps Pay twice,
    // or when they are retrying after a previous FAILED transaction.
    ...
    // Nothing sent to Daraja yet — safe to throw with no side-effects.
    throw new MpesaError(
      'Database error creating subscription record',
      'DB_ERROR',
      dbErr,
    );
  }

  // ── Step 3: Fire the STK push (fully outside any DB transaction) ──────────
  // If this fails the Subscription sits as INCOMPLETE — harmless, and reused
  // on the next retry attempt (see find-first check above).
  ...
  // ── Step 4: Create MpesaTransaction now that Daraja IDs are available ─────
  // Both checkoutRequestId and merchantRequestId are present so the insert
  // satisfies the schema's non-null constraints without any migration.
  // If this write fails after the STK push has already succeeded we log
  // loudly (the webhook can reconcile via checkoutRequestId) but do not
  // throw — the user's phone was already prompted and we must not 500.
  ...
  try {
    ...
    console.error(
      '[mpesa] CRITICAL: STK push succeeded but MpesaTransaction create failed. ' +
      'Webhook reconciliation required via checkoutRequestId.',
      { subscriptionId: subscription.id, stkResponse, error: dbErr },
    );
    // Do not re-throw — return the result so the client can poll for status.
  }

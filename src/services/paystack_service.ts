...
export async function getOrCreatePaystackCustomer(
  ...
  await prismaClient.user.updateMany({
    where: { id: userId, paystackCustomerCode: null },
  ...
  return (updated?.paystackCustomerCode ?? customer.customer_code) as string;
}
...
export async function findPaystackSubscriptionByCustomer(
  ...
  const payload = await paystackRequest<any>(
    'GET',
    `/subscription?customer=${encodeURIComponent(customerCode)}&status=active&perPage=50&page=1`,
  );
  // AUDIT-FIX-8: Paystack subscription lookup now filters to active subscriptions server-side.
  ...
  const matches = list.filter(s => {
    ...
    // AUDIT-FIX-8: Customer matching now uses Paystack's customer object code only.
    const planMatches = !planCode || s?.plan?.plan_code === planCode || s?.plan === planCode;
    const hasCodes = !!s?.subscription_code && !!s?.email_token;
    // SECURITY: Paystack's &status=active query param is silently ignored by their
    // list-subscription endpoint — it is not a documented filter. Must enforce client-side.
    // Only 'active' and 'non-renewing' subscriptions are still billing.
    // Matching 'cancelled' or 'completed' codes here would set the DB row to ACTIVE
    // while Paystack never charges the user again — free premium access indefinitely.
    const isLive = s?.status === 'active' || s?.status === 'non-renewing';
    return customerMatches && planMatches && hasCodes && isLive;
  });
...
async function createPaymentOnceForReference(params: {
  ...
  // FIX-3: Never write a zero-amount succeeded payment record — it corrupts billing
  // history and breaks any future reconciliation that sums amounts. Matches the
  // same guard in createPaymentOnce (webhook path).
  if (params.amountCents <= 0) {
    console.warn(
      `[createPaymentOnceForReference] Skipping zero-amount record for ref ${params.reference} ` +
...
async function storeCodesAndActivate(params: {
  ...
  if (['CANCELLED', 'EXPIRED', 'INCOMPLETE_EXPIRED'].includes(String(existing.status))) {
    throw new Error(`Refusing to activate terminal subscription ${params.subscriptionId} with status ${existing.status}`);
  }
  ...
  await prisma.subscription.update({
    ...
    data: {
      ...
      // `undefined` is intentional — Prisma silently skips undefined fields in
      // update(), preserving the existing activatedAt on renewals.
      // DO NOT change to null; that would actively erase the original activation date.
...
export async function verifyPaystackPayment(reference: string, userId: string): Promise<VerifyPaymentResult> {
  ...
  if (!sub.plan) {
    return {
      ...
      message: 'Subscription plan record missing — cannot verify amount. Contact support.',
    };
  }
...
async function fetchCurrentSub(userId: string): Promise<CurrentSubscription | null> {
  ...
  const scheduledPlan = sub.scheduledPlanId
    ? await prisma.plan.findUnique({ where: { id: sub.scheduledPlanId }, select: { slug: true } })
    : null;
  ...
  return {
    ...
    scheduledPlanSlug: scheduledPlan?.slug ?? null,
...
async function activatePendingPaystackSubscriptionFromCodes(params: {
  ...
  // FIX-1: Guard against reactivating a row that was legitimately cancelled/expired
  // between when the reconcile candidate list was fetched and now (cron race).
  // storeCodesAndActivate (webhook path) has this guard; this function must match it.
  if (['CANCELLED', 'EXPIRED', 'INCOMPLETE_EXPIRED'].includes(sub.status)) {
    console.error(
      `[activatePendingFromCodes] Refusing to activate terminal subscription ` +
      `${params.subscriptionId} (status=${sub.status}, source=${params.source}). ` +
      `This may indicate a reconciliation race with the expiry job.`,
    );
    return;
  }

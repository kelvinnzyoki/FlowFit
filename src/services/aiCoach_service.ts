...
class AICoachService {
  ...
  private async buildUserContext(userId: string): Promise<UserContext> {
    ...
    if (enrollment) {
      ...
      const weekData        = enrollment.program.weeks.find((w: any) => w.weekNumber === currentWeek);
      const dayData         = weekData?.days?.find((d: any) => d.dayNumber === currentDay);
      const nextExercises   = dayData?.exercises
        ?.map((de: any) => de.exercise?.name ?? de.exerciseName ?? 'Exercise')
        .filter(Boolean) ?? [];
      program = {
        ...
        nextExercises: (dayData?.exercises ?? []).map((de: any) => de.exercise?.name ?? de.exerciseName ?? '').filter(Boolean),
      };
    }

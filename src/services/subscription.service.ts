...
/**
 * BUG-1 FIX — priority-sorted selection.
 *
 * The previous implementation used findFirst({ orderBy: { createdAt: 'desc' } })
 * which returns the NEWEST record regardless of status.  This causes a false
 * "payment failed" banner whenever a checkout creates an INCOMPLETE row
 * (newer than the user's real ACTIVE/TRIALING subscription) or a previous
 * failed attempt left a PAST_DUE row more recent than the active one.
 *
 * Fix: fetch up to 10 subscriptions for the user, rank them by a status
 * priority map (ACTIVE=0, TRIALING=1, PAST_DUE=2, …, INCOMPLETE=5,
 * CANCELLED/EXPIRED=6), and return the best one.  Among subscriptions of
 * equal priority, the most recently created wins (the original tie-break).
 */
export async function getCurrentSubscription(
  userId: string,
): Promise<CurrentSubscription | null> {
  // Fetch the most recent 10 subscriptions so we can pick the best one.
  // 10 is far more than any real user will ever have; it avoids a full-scan.
  ...
  // Priority: lower number = better.  We want the most "alive" subscription.
  ...
  const ranked = [...rows].sort((a, b) => {
    ...
    // Same priority — prefer the most recently created (original behaviour)
...
export async function createMpesaSubscription(
  ...
  // Initiate STK Push — mpesa.service reads MPESA_CALLBACK_URL from env directly

...
async function notifySubscriptionAction(
  userId: string,
  type: string,
  title: string,
  body: string,
  icon: string = '💳',
  link: string = 'subscription.html',
): Promise<void> {
  try {
    ...
    await prisma.notification.create({
      data: { userId, type, title, body, icon, link },
    });
  } catch (err) {
    // Notification failure must never break billing state changes.
    console.error(`[notifySubscriptionAction] Failed for user ${userId}:`, err);
  }
}

function formatPlanName(plan: { name?: string | null; slug?: string | null } | null | undefined): string {
  return plan?.name || plan?.slug || 'your plan';
}

function formatDateForUser(date: Date | null | undefined): string {
  if (!date) return 'your current period end';
...
async function applyPendingDowngradeIfDue(userId: string, ipAddress?: string): Promise<void> {
  const now = new Date();
...
export async function getCurrentSubscription(
  ...
  try {
    await applyPendingDowngradeIfDue(userId);
...
export async function cancelSubscription(
  ...
  if (hasPaystackSub) {
    if (immediately) {
      ...
      await notifySubscriptionAction(
        sub.userId,
        'subscription_cancelled',
        'Subscription cancelled',
        `${formatPlanName(sub.plan)} has been cancelled immediately.`,
        '❌',
      );
    } else {
      ...
      await notifySubscriptionAction(
        ...
        `${formatPlanName(sub.plan)} will stay active until ${formatDateForUser(sub.currentPeriodEnd)}. No further renewals will be made.`,
        '⏳',
      );
    }
  } else {
...
export async function reactivateSubscription(
  ...
  if (hasPaystackSub) {
    ...
    if (needsNewPaystackSub) {
      ...
      // Persist new codes immediately so the subscription.create webhook
      // can locate this row when it fires.
      await prisma.subscription.update({
        where: { id: sub.id },
        data:  {
          paystackSubscriptionCode: subscriptionCode,
  ...
  const plan = await prisma.plan.findUnique({ where: { id: sub.planId }, select: { name: true, slug: true } });
  await notifySubscriptionAction(
    sub.userId,
    'subscription_reactivated',
    'Subscription reactivated',
    `${formatPlanName(plan)} will renew normally.`,
    '✅',
  );
...
export async function scheduleDowngrade(
  ...
  // Mark the subscription as a scheduled downgrade BEFORE calling Paystack disable.
  // Paystack can send subscription.disable immediately; the webhook must see
  // scheduledPlanId/cancellationReason and avoid sending a cancellation notification.
  await prisma.subscription.update({
    where: { id: sub.id },
    data: {
  ...
  try {
    ...
    console.error('[scheduleDowngrade] paystackDisable failed:', err);
    // Roll back the local scheduled downgrade because remote auto-renew was not stopped.
    await prisma.subscription.update({
      where: { id: sub.id },
      data: {
        scheduledPlanId:    null,
    ...
    throw new Error('Could not schedule downgrade on Paystack. Please try again.');
  }
  ...
  await notifySubscriptionAction(
    ...
    'Downgrade scheduled',
    `${formatPlanName(sub.plan)} will change to ${formatPlanName(newPlan)} on ${formatDateForUser(sub.currentPeriodEnd)}.`,
    '⬇️',
  );

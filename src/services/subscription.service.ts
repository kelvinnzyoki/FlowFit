...
export async function getBillingPortalUrl(userId: string): Promise<string> {
  const sub = await prisma.subscription.findFirst({
    ...
    orderBy: { createdAt: 'desc' },
    select:  { id: true, paystackEmailToken: true, paystackSubscriptionCode: true },
  });

  let emailToken = sub?.paystackEmailToken ?? null;

  // If emailToken is missing, fetch live from Paystack (subscription.create webhook may have been missed)
  if (!emailToken && sub?.paystackSubscriptionCode) {
    try {
      ...
      console.error('[getBillingPortalUrl] Failed to fetch emailToken from Paystack:', err);
    }
  }

  if (!emailToken) {
    throw new Error('No Paystack subscription management token found. The subscription may not be fully activated yet.');
  }

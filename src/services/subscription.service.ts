// SECURITY PATCHES APPLIED: FIX-1, FIX-2
// ISSUE-8: WARNING — paystack_subscription_service.ts contains a parallel
// cancel/reactivate implementation that throws immediately on missing paystackEmailToken
// (no fetch-from-Paystack fallback). Ensure NO routes import cancelSubscription or
// reactivateSubscription from that file. All imports must come from THIS file.
/**
...
async function getReusablePaystackAuthorization(customerCode: string): Promise<string | null> {
  const customerData = await paystackRequest<any>(
    'GET',
    `/customer/${encodeURIComponent(customerCode)}`,
  ).catch(() => null);

  const authorizations: any[] = customerData?.authorizations ?? [];
  const now = new Date();

  const valid = authorizations.find((auth: any) => {
    if (!auth?.reusable || auth?.channel !== 'card') return false;
    const expYear  = Number(auth.exp_year  ?? 0);
    const expMonth = Number(auth.exp_month ?? 0);
    if (!expYear || !expMonth) return true;
    return new Date(expYear, expMonth, 1) > now;
  });

  return valid?.authorization_code ?? null;
}

export async function applyPendingDowngradeIfDue(userId: string, ipAddress?: string): Promise<void> {
  const now = new Date();
  ...
  const sub = await prisma.$transaction(async (tx) => {
    ...
    if (!claimed?.scheduledPlanId) return null;
  ...
  // FIX-6: sub.scheduledPlanId here is the value from BEFORE the transaction cleared it.
  // The transaction atomically claimed the row (set scheduledPlanId=null in DB) and
  // returned the pre-update object so we still have the target plan ID to work with.
  // This is intentional — do NOT refactor to re-read from DB here or the plan ID is gone.
  if (!sub?.scheduledPlanId) return;

  const targetPlan = await prisma.plan.findUnique({ where: { id: sub.scheduledPlanId } });
  if (!targetPlan) {
    await prisma.subscription.update({
      ...
      data: {
        ...
        autoRenew: false,
        cancellationReason: 'pending_downgrade_target_plan_missing',
      },
    });
    await logEvent(sub.id, 'EXPIRED', sub.status, 'EXPIRED', { reason: 'pending_downgrade_target_plan_missing' }, ipAddress);
    return;
  }

  const newInterval = (sub.scheduledInterval ?? sub.interval) as BillingInterval;
  ...
  if (sub.provider === 'PAYSTACK') {
    if (!sub.paystackCustomerCode) {
      throw new Error('Cannot apply pending downgrade: Paystack customer code is missing.');
    }

    const planCode = newInterval === 'YEARLY'
      ? (targetPlan as any).paystackPlanCodeYearly
      : (targetPlan as any).paystackPlanCodeMonthly;

    if (!planCode) {
      throw new Error('Cannot apply pending downgrade: target Paystack plan code is missing.');
    }

    const authorizationCode = await getReusablePaystackAuthorization(sub.paystackCustomerCode);
    if (!authorizationCode) {
      await prisma.subscription.update({
        ...
        data: {
          ...
          cancelAtPeriodEnd: false,
          autoRenew: false,
          cancellationReason: 'pending_downgrade_card_missing_or_expired',
        },
      });
      await logEvent(sub.id, 'EXPIRED', sub.status, 'EXPIRED', { reason: 'pending_downgrade_card_missing_or_expired' }, ipAddress);
      return;
    }

    // ISSUE-7: createPaystackSubscription and the DB update must be treated as a unit.
    // If Paystack succeeds but the DB update fails, a live Paystack subscription exists
    // with no DB row. On the next run, applyPendingDowngradeIfDue creates a second one
    // and the user is double-billed. Compensate by disabling the orphaned Paystack
    // subscription if the DB write fails.
    ...
    newPaystackSubscriptionCode = created.subscription_code;
    newPaystackEmailToken       = created.email_token;
  }

  const nextPeriodEnd = new Date(now);
  if (newInterval === 'YEARLY') nextPeriodEnd.setFullYear(nextPeriodEnd.getFullYear() + 1);
  ...
  try {
    await prisma.subscription.update({
      ...
      data: {
        ...
        ...(newPaystackSubscriptionCode ? { paystackSubscriptionCode: newPaystackSubscriptionCode } : {}),
        ...(newPaystackEmailToken ? { paystackEmailToken: newPaystackEmailToken } : {}),
      },
    });
  } catch (dbErr) {
    // DB write failed after Paystack subscription was already created.
    // Disable the orphaned Paystack subscription to prevent a double-charge
    // on the next billing cycle, then rethrow so the caller can alert/retry.
    if (newPaystackSubscriptionCode && newPaystackEmailToken) {
      await paystackDisable(newPaystackSubscriptionCode, newPaystackEmailToken).catch((disableErr: any) => {
        console.error(
          `[applyPendingDowngradeIfDue] CRITICAL: DB update failed AND compensating ` +
          `paystackDisable also failed for sub ${sub.id} code ${newPaystackSubscriptionCode}. ` +
          `Manual intervention required to avoid double-billing.`,
          disableErr,
        );
      });
    }
...
export async function getCurrentSubscription(
  ...
  try {
    // AUDIT-FIX-3: Pending downgrades are processed by cron-reconcile, not silently during reads.
    ...
    // FIX-TRIAL-2: Do NOT mask a pending-activation INCOMPLETE row as 'TRIALING'.
    // The previous masquerade, combined with trialEndsAt being set on the checkout
    // row (FIX-TRIAL-1 above), caused the frontend to render "Free trial activated"
    // for every paid checkout while the subscription.create webhook was in-flight.
    // isPendingActivation is still returned so the frontend can show
    // "Payment processing — activation in progress" UI without misreading the status.
    ...
    const effectiveStatus = sub.status; // never masquerade as TRIALING

    // Assertion: paystackSubscriptionCode must NEVER be sent to clients.
    // email_token is the second auth factor for Paystack disable/enable — exposure
    // allows anyone to cancel or reactivate a subscription without going through auth.
    if (sub.paystackSubscriptionCode) {
      // Value exists in DB — confirm it is stripped before leaving this function.
      console.assert(
        false,
        '[getCurrentSubscription] paystackSubscriptionCode is present on the DB row ' +
        `(sub ${sub.id}) but must be null in the response. Stripping.`,
      );
    }
    // AUDIT-FIX-12: Runtime assertion protects against leaking Paystack subscription codes.
    ...
    return {
      ...
      // FIX-11: paystackSubscriptionCode is intentionally returned as null to the client.
      // Paystack's POST /subscription/manage/email/ only requires the subscription code,
      // meaning anyone who intercepts the real value can trigger Paystack management emails
      // to the user's inbox (containing card-update and cancellation links) without going
      // through our auth layer. Null satisfies the CurrentSubscription type contract while
      // keeping the real code backend-only. The frontend uses /subscriptions/billing-portal
      // (server-side) to generate scoped management links.
      paystackSubscriptionCode: null,
      hasPaystackCodes:         !!(sub.paystackSubscriptionCode && sub.paystackEmailToken),
      isPendingActivation,
    };
...
export async function createCheckoutSession(
  ...
  const pendingSub = await prisma.subscription.create({
    data: {
      ...
      cancelAtPeriodEnd:   false,
      autoRenew:           true,
      // FIX-TRIAL-1: Trial fields are intentionally NOT set here.
      // createCheckoutSession is the paid path; free trials are created exclusively
      // by POST /subscriptions/trial.  Setting trialEndsAt on an INCOMPLETE checkout
      // row caused getCurrentSubscription to return effectiveStatus='TRIALING' while
      // the Paystack payment was pending, making the frontend render "Free trial
      // activated" instead of "Payment processing — activation in progress".
    },
  });
...
export async function handleMpesaSuccess(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // FIX-5 (was FIX-2): Validate received amount against plan price.
    // Previously used `typeof amountKes === 'number'` as the gate, which allowed a
    // bypass: if the Safaricom callback omitted the Amount field, amountKes was
    // undefined, the typeof check was false, validation was silently skipped, and a
    // zero-KES payment activated a full subscription.
    // Fix: treat a missing or non-positive amountKes as an invalid payment and throw.
    const expectedKes =
      mpesaTx.subscription.interval === 'YEARLY'
        ? (mpesaTx.subscription as any).plan?.mpesaYearlyKes
        : (mpesaTx.subscription as any).plan?.mpesaMonthlyKes;

    if (!amountKes || amountKes <= 0) {
      throw new Error(
        `[handleMpesaSuccess] Missing or zero amount received for subscription ` +
        `${mpesaTx.subscriptionId}. Expected KES ${expectedKes ?? 'unknown'}.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `[handleMpesaSuccess] Amount mismatch: received KES ${amountKes}, ` +
        `expected KES ${expectedKes} for subscription ${mpesaTx.subscriptionId}`
      );
    }
...
export async function runMpesaRenewals(): Promise<{
  ...
  for (const sub of subs) {
    try {
      // ISSUE-6: Guard against duplicate STK pushes if cron overlaps itself.
      // If a PENDING transaction exists and hasn't timed out, skip this sub.
      // Without this, two overlapping cron runs each send an STK prompt and the
      // user risks being charged twice for the same renewal cycle.
      ...
      if (alreadyPending) {
        console.log(`[cron/renewals] Sub ${sub.id}: STK already PENDING (${alreadyPending.checkoutRequestId}) — skipping`);
        continue;
      }
...
export async function runRetries(): Promise<{
  ...
  let errors    = 0;
   ...
   for (const sub of subs) {
    try {
      const alreadyPending = await prisma.mpesaTransaction.findFirst({
        where: { subscriptionId: sub.id, status: 'PENDING', timeoutAt: { gt: now } },
      });
      if (alreadyPending) {
        console.log(`[cron/retries] Sub ${sub.id}: STK already PENDING — skipping`);
        continue;
      }
...
export async function cancelSubscription(
  ...
  const sub = await prisma.subscription.findFirst({
    ...
    // FIX-4: INCOMPLETE included so abandonCheckout (immediately=true) can cancel a
    // Paystack checkout row before subscription.create fires. Without this, the row
    // survives, Paystack retries the webhook for up to 72 hours, and eventually
    // activates and bills a subscription the user tried to abandon.
    orderBy: { createdAt: 'desc' },
    include: { plan: true },
  });
  ...
  if (hasPaystackSub) {
    if (immediately) {
      // Immediate cancel: set CANCELLED in DB first, then disable on Paystack.
      // A DB-first update is recoverable; a Paystack-first cancel can leave DB ACTIVE.
      await prisma.subscription.update({
        where: { id: sub.id },
        data: {
          status:             'CANCELLED',
      ...
      try {
        ...
        console.error(
          `[CRITICAL] cancelSubscription immediately=true: DB row ${sub.id} is ` +
          `CANCELLED but paystackDisable failed for code ` +
          `${sub.paystackSubscriptionCode}. Manual Paystack disable required ` +
          `before next billing date to prevent unwanted charge.`,
          disableErr,
        );
      }
      // AUDIT-FIX-2: Immediate cancellation no longer leaves a thrown Paystack disable error unreported to ops.
...
export async function reactivateSubscription(
  ...
  // FIX-REACTIVATE-2: Also search CANCELLED rows still within their paid period.
  // The subscription.disable webhook previously set status=CANCELLED on period-end-
  // cancel rows before FIX-REACTIVATE-1 was deployed.
  ...
  if (!sub) {
    const expiredCancelled = await prisma.subscription.findFirst({
      where: { userId, cancelAtPeriodEnd: true, status: { in: ['CANCELLED', 'EXPIRED'] } },
      orderBy: { currentPeriodEnd: 'desc' },
    });
    if (expiredCancelled) {
      throw new Error(
        'Your subscription period has ended and can no longer be reactivated. ' +
  ...
  if (hasPaystackSub) {
    // FIX-REACTIVATE-3: Paystack permanently cancels a subscription when
    // POST /subscription/disable is called — even if called for "cancel at period end".
    // POST /subscription/enable subsequently fails with:
    //   "Subscription has been cancelled, and cannot be reactivated"
    //
    // Strategy:
    //   1. Fetch the live Paystack status so we know what we're dealing with.
    //   2. If "active" → already live on Paystack (disable failed silently), skip enable.
    //   3. If "non-renewing" → try enable. If Paystack still refuses (e.g. internal
    //      state mismatch), fall through to step 4.
    //   4. Any other state (cancelled, completed) OR failed enable → create a brand-new
    //      Paystack subscription on the same plan/customer, with start_date set to the
    //      existing currentPeriodEnd. This means no double-charge: Paystack will first
    //      bill the user on their original renewal date and auto-renew from there.
    //      The DB row keeps its ID; only the codes are replaced.

    const liveSub = await fetchPaystackSubscription(subscriptionCode!).catch(() => null);

    if (!liveSub) {
      throw new Error(
        'Could not reach Paystack to verify subscription status. Please try again in a moment.',
      );
    }

    // Refresh email_token from the live response before any enable attempt.
    if (liveSub.email_token && liveSub.email_token !== emailToken) {
      emailToken = liveSub.email_token;
      // Persisted together with the final reactivation update below.
    }

    let needsNewPaystackSub = false;

    if (liveSub.status === 'active') {
      // Already live — no API call required.
      console.log(`[reactivateSubscription] Sub ${sub.id} already active on Paystack — skipping enable`);
    } else if (liveSub.status === 'non-renewing') {
      try {
        await paystackEnable(subscriptionCode!, emailToken!);
      } catch (enableErr: any) {
        // Paystack refuses even for non-renewing state (confirmed behaviour: once
        // disabled via API the subscription is permanently cancelled on their side).
        console.warn(
          `[reactivateSubscription] paystackEnable rejected on non-renewing sub ${subscriptionCode}: ` +
          (enableErr?.message ?? enableErr),
        );
        needsNewPaystackSub = true;
      }
    } else {
      // cancelled, completed, attention — cannot be re-enabled.
      needsNewPaystackSub = true;
    }

    if (needsNewPaystackSub) {
      // Create a replacement Paystack subscription that picks up on the user's
      // existing renewal date so they are not charged twice for the current period.
      if (!sub.paystackCustomerCode) {
        throw new Error(
          'Cannot reactivate: Paystack customer code is missing. Please start a new subscription.',
        );
      }
      ...
      if (!planCode) {
        throw new Error(
          'Cannot reactivate: Paystack plan code is not configured for this plan. Please contact support.',
        );
      }

      if (!sub.currentPeriodEnd) {
        throw new Error(
          'Cannot reactivate: subscription has no renewal date on record. Please start a new subscription.',
        );
      }

      // FIX-6: Verify the customer has a usable card before creating the new
      // subscription. Paystack creates the subscription object and returns codes
      // even when no valid authorization exists, but the first charge silently
      // fails — giving the user ACTIVE status without you being paid.
      // Fetch the customer and confirm at least one non-expired authorization exists.
      const customerData = await paystackRequest<any>(
        'GET',
        `/customer/${encodeURIComponent(sub.paystackCustomerCode!)}`,
      ).catch(() => null);

      const authorizations: any[] = customerData?.authorizations ?? [];
      const now2 = new Date();
      const hasValidCard = authorizations.some((auth: any) => {
        if (!auth?.reusable || auth?.channel !== 'card') return false;
        const expYear  = Number(auth.exp_year  ?? 0);
        const expMonth = Number(auth.exp_month ?? 0);
        if (!expYear || !expMonth) return true; // no expiry data — assume valid
        // Card expires at the END of exp_month
        const expiry = new Date(expYear, expMonth, 1); // first day of month after expiry
        return expiry > now2;
      });

      if (!hasValidCard) {
        throw new Error(
          'Your saved card appears to have expired or been removed. ' +
          'Please update your payment method before reactivating.',
        );
      }

      // Guard: if currentPeriodEnd is already in the past, passing it as start_date
      // causes Paystack to charge immediately (they treat a past date as "now").
      // In that case omit start_date and let Paystack charge today for a new cycle.
      const startDate = sub.currentPeriodEnd > now2 ? sub.currentPeriodEnd : undefined;

      const newPaystackSub = await createPaystackSubscription(
        sub.paystackCustomerCode!,
        planCode,
        undefined,   // Paystack uses the customer's most recent reusable authorization
        startDate,   // Future date = no double-charge; undefined = charge now for new cycle
      );

      subscriptionCode = newPaystackSub.subscription_code;
      emailToken       = newPaystackSub.email_token;

      // Persist new codes together with the final reactivation update below.
    }
  }

  const wasIncorrectlyCancelled = sub.status === 'CANCELLED';
  ...
  await prisma.subscription.update({
    ...
    data: {
      ...
      cancellationReason:       null,
      autoRenew:                true,
      // FIX-REACTIVATE-2: Restore ACTIVE if the old webhook bug set this row to CANCELLED.
      ...(wasIncorrectlyCancelled ? { status: 'ACTIVE', cancelledAt: null } : {}),
    },
  });

  await logEvent(
    sub.id, 'REACTIVATED', sub.status, 'ACTIVE',
    {
      hadPaystackSub:           hasPaystackSub,
      recoveredPaystackCodes:   hasPaystackSub && (!sub.paystackSubscriptionCode || !sub.paystackEmailToken),
...
export async function scheduleDowngrade(
  ...
  if (!newPlan) throw new Error('Target plan not found');
  if (String((newPlan as any).slug).toLowerCase() === 'free') {
    throw new Error('Use the cancel button to move to the Free plan.');
  }
  if (!sub.currentPeriodEnd) {
    throw new Error('Cannot schedule downgrade: current subscription has no period end.');
  }
  ...
  if (sub.provider === 'PAYSTACK' && sub.paystackSubscriptionCode && !emailToken) {
    try {
      ...
      console.error('[scheduleDowngrade] Could not fetch emailToken from Paystack:', err);
    }
  }
  ...
  await prisma.subscription.update({
    ...
    data: {
      ...
      cancelAtPeriodEnd: true,
      autoRenew:         false,
      cancellationReason:'scheduled_downgrade',
    },
  });

  if (sub.provider === 'PAYSTACK' && sub.paystackSubscriptionCode && emailToken) {
    await paystackDisable(sub.paystackSubscriptionCode, emailToken).catch(err => {
      console.error('[scheduleDowngrade] paystackDisable failed — scheduled in DB, will retry via cron:', err);
    });
  }
    ...
    try {
      ...
      await prisma.$transaction(async (db) => {
        ...
        await db.subscriptionLog.create({
          data: {
            subscriptionId: sub.id,
            event:          paymentExists ? 'RECONCILED' : 'PAYMENT_SUCCEEDED',

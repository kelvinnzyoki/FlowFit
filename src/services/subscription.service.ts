...
export async function createCheckoutSession(
  ...
  try {
    txn = await initializeTransaction({
      ...
      currency: 'KES',
      // Deterministic Paystack subscription strategy:
      // Do NOT pass plan here. This first charge is a normal one-time card payment.
      // After transaction/verify succeeds, paystack.service.ts creates the remote
      // subscription with POST /subscription using txn.authorization.authorization_code
      // and start_date = next billing date. That endpoint immediately returns
      // subscription_code + email_token, so the DB does not depend on delayed
      // automatic subscription.create webhooks.

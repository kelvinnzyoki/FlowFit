...
export async function createCheckoutSession(
  ...
  // ── Subscription guard ────────────────────────────────────────────────────
  // A genuinely paid subscription (ACTIVE/TRIALING with a paystackSubscriptionCode
  // or an MPESA provider) must not be duplicated — user should upgrade instead.
  //
  // A FREE TRIAL (TRIALING + no paystackSubscriptionCode + no mpesa provider)
  // has no payment on file and must NOT block a new paid checkout. We cancel it
  // first so the webhook only has one INCOMPLETE row to activate.
  //
  // A FREE-PLAN subscription (plan.slug === 'free') is similarly non-blocking.
  ...
  if (activeAnySub) {
    const isPaidPaystack =
      activeAnySub.status === 'ACTIVE' &&
      !!activeAnySub.paystackSubscriptionCode;

    const isPaidMpesa =
      activeAnySub.provider === 'MPESA' &&
      activeAnySub.status === 'ACTIVE';

    const isFreePlan =
      (activeAnySub.plan as any)?.slug === 'free';

    const isFreeTrial =
      activeAnySub.status === 'TRIALING' &&
      !activeAnySub.paystackSubscriptionCode &&
      activeAnySub.provider !== 'MPESA';

    if (isPaidPaystack || isPaidMpesa) {
      // Genuine paid subscription — block and tell the client to upgrade instead
      throw new Error(
        `You already have an active ${activeAnySub.provider} subscription. ` +
        'Use Upgrade or Downgrade instead.',
      );
    }

    if (isFreeTrial || isFreePlan) {
      // No payment was ever taken — cancel the placeholder so the new paid
      // INCOMPLETE row becomes the only active subscription after the webhook fires.
      await prisma.subscription.update({
        where: { id: activeAnySub.id },
        data: {
          status:             'CANCELLED',
          cancelledAt:        new Date(),
          cancellationReason: 'Replaced by paid checkout',
      ...
      await logEvent(
        ...
        activeAnySub.status,
        'CANCELLED',
        { reason: 'replaced_by_paid_checkout', newPlanId: planId },
      );
    }
    // Any other status (INCOMPLETE from an abandoned checkout etc.) — fall through
    // and let the new INCOMPLETE row replace it via the webhook.
  }

  // Also clean up any stale INCOMPLETE rows for this user so they don't
  // accumulate from abandoned checkouts.
...
export async function createMpesaSubscription(
  ...
  // NEW-R4: Block genuine paid duplicates but allow free trials / free plan to
  // proceed (same logic as createCheckoutSession above).
  const activeAnySub = await prisma.subscription.findFirst({
    where:   { userId, status: { in: ['ACTIVE', 'TRIALING'] } },
    include: { plan: true },
    orderBy: { createdAt: 'desc' },
  ...
  if (activeAnySub) {
    ...
    const isFreePlan  = (activeAnySub.plan as any)?.slug === 'free';
    const isFreeTrial =
      activeAnySub.status === 'TRIALING' &&
      !activeAnySub.paystackSubscriptionCode &&
      activeAnySub.provider !== 'MPESA';
    ...
    if (isFreeTrial || isFreePlan) {
      await prisma.subscription.update({
        ...
        data: {
          status:             'CANCELLED',
          cancelledAt:        new Date(),
          cancellationReason: 'Replaced by M-Pesa checkout',
          cancelAtPeriodEnd:  false,
        },
      });
      await logEvent(
        ...
        activeAnySub.status,
        'CANCELLED',
        { reason: 'replaced_by_mpesa_checkout', newPlanId: planId },
      );
    }
  }

  // Clean up stale INCOMPLETE M-Pesa rows for this user/plan
  await prisma.subscription.updateMany({
    where: { userId, status: 'INCOMPLETE', planId, provider: 'MPESA' },
    data: {
      status:             'INCOMPLETE_EXPIRED',
      cancellationReason: 'Superseded by new M-Pesa attempt',
    },
  });

...
export async function cancelSubscription(
  ...
  if (hasPaystackSub) {
    if (immediately) {
      ...
      // Cancel at period end — do NOT call paystackDisable now.
      // Paystack's /subscription/disable fires its own webhook (subscription.disable)
      // which sets status=CANCELLED in our DB immediately — user loses access today.
      // Correct: flag DB only. Expiry cron calls paystackDisable at currentPeriodEnd.
      await prisma.subscription.update({
        where: { id: sub.id },
        data: {

...
async function fetchAndStorePaystackCodes(params: {
  subscriptionId: string;
  customerCode: string;
  attempts?: number;
  delayMs?: number;
}): Promise<boolean> {
  const { subscriptionId, customerCode, attempts = 6, delayMs = 2_000 } = params;
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      if (found?.subscription_code && found?.email_token) {
        ...
        console.log(
          `[verifyPaystackPayment] Stored Paystack management codes for sub ${subscriptionId} ` +
          `on attempt ${attempt}.`,
        );
        return true;
      }
      ...
      console.warn(
        `[verifyPaystackPayment] Code fetch attempt ${attempt}/${attempts} failed:`,
        err?.message ?? err,
      );
    }
  ...
  console.warn(
    `[verifyPaystackPayment] Paystack management codes are not available yet for sub ${subscriptionId}. ` +
    'The subscription remains ACTIVE; subscription.create webhook should backfill the codes later.',
  );
  return false;
}
...
export async function verifyPaystackPayment(
  ...
  if (sub.status === 'ACTIVE') {
    // Already activated (webhook beat us or prior poll). Still try to backfill
    // management codes during this foreground request if they are missing.
    if ((!sub.paystackSubscriptionCode || !sub.paystackEmailToken) && sub.paystackCustomerCode) {
      await fetchAndStorePaystackCodes({
        subscriptionId: sub.id,
        customerCode:   sub.paystackCustomerCode,
      });
    }
    return { success: true, status: 'success', subscription: await fetchCurrentSub(userId) };
  }
...
// ── Post-activation: fetch subscription_code + email_token ───────────────
  // These fields are needed for Paystack management actions only (cancel,
  // enable/disable, billing portal). They must not block user entitlement.
  // Paystack often creates them shortly after the successful charge, so do a
  // bounded synchronous poll here. Do not use background setInterval/setTimeout
  // retries on serverless runtimes; they are not reliable after the response.
  if ((!subscriptionCode || !emailToken) && sub!.paystackCustomerCode) {
    await fetchAndStorePaystackCodes({
      subscriptionId: sub!.id,
      customerCode:   sub!.paystackCustomerCode,
    });

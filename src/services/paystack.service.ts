...
export async function verifyPaystackPayment(reference: string, userId: string): Promise<VerifyPaymentResult> {
  ...
  // First try to recover an already-created Paystack subscription. This protects
  // against retries and webhook/verify races.
  ...
  // Deterministic fallback: if this checkout was initialized as a normal card
  // payment, Paystack will not automatically create a subscription. Create it
  // now using the reusable authorization from the successful transaction.
  // start_date is set to nextPaymentDate so the customer is not charged twice
  // immediately; their first paid period is the transaction that just succeeded.
  if ((!subscriptionCode || !emailToken) && customerCode && planCode) {
    ...
    if (authorizationCode) {
      const created = await createPaystackSubscription(
        ...
        console.error('[verifyPaystackPayment] POST /subscription failed:', err?.message ?? err);
        return null;
      });
      ...
      console.error('[verifyPaystackPayment] Missing authorization_code; cannot create Paystack subscription deterministically', {
        reference,
        hasAuthorization: !!txn.authorization,
      });
    }
  }

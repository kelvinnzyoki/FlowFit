...
export async function fetchPaystackCustomer(customerCode: string): Promise<PaystackCustomer> {
  return paystackRequest<PaystackCustomer>('GET', `/customer/${encodeURIComponent(customerCode)}`);
}
...
/**
 * @deprecated
 * DO NOT CALL from any webhook handler, verify route, or checkout path.
 *
 * When checkout is initialized with a Paystack plan_code, Paystack creates the
 * subscription automatically and fires subscription.create. Calling this function
 * additionally creates a SECOND subscription on Paystack, causing a double-charge
 * on the next billing cycle. This was the root cause of FIX-PS-1.
 *
 * This function is retained only for direct admin scripts run outside the request
 * lifecycle. It must never be imported into route or service files.
 */
...
export async function verifyPaystackPayment(reference: string, userId: string): Promise<VerifyPaymentResult> {
  ...
  if (expectedAmountKobo > 0 && Number(txn.amount) < expectedAmountKobo) {
    return {
      success: false,
      status: 'amount_mismatch',
      message:
        `Payment amount ${txn.amount} kobo is below the required ` +
        `${expectedAmountKobo} kobo for this plan.`,
    };
  }

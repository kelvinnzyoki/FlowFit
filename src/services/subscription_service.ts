...
/**
 * Start a no-payment free trial.
 * Creates a TRIALING subscription in DB only — no Stripe/M-Pesa charge.
 * User gets plan access for plan.trialDays days. If they don't add payment
 * before trialEndsAt, getCurrentSubscription auto-expires it to CANCELLED.
 */
export async function startTrial(
  ...
  if (!plan) throw new Error('Plan not found');
  if (!plan.isActive) throw new Error('Plan is no longer available');
  if (plan.monthlyPriceCents === 0) throw new Error('Free plan does not have a trial');
  if (plan.trialDays <= 0) throw new Error('This plan does not offer a free trial');
  ...
  if (existing) throw new Error('You already have an active subscription. Upgrade or downgrade instead.');

  // Prevent trial abuse — one trial per plan per user lifetime
  const previousTrial = await prisma.subscription.findFirst({
    where: { userId, planId, status: { in: ['CANCELLED', 'EXPIRED'] }, trialEndsAt: { not: null } },
  });
  if (previousTrial) throw new Error(`You have already used the free trial for ${plan.name}. Please subscribe directly.`);
  ...
  await logEvent(sub.id, 'TRIAL_STARTED', null, 'TRIALING', {
    planSlug:    plan.slug,
    trialDays:   plan.trialDays,
...
export async function upgradeSubscription(
  ...
  // M-Pesa subscription — no Stripe sub to modify. User should pay via
  // Stripe checkout (creates a new Stripe sub) or M-Pesa (handled by M-Pesa flow).
  // This endpoint is only for Stripe↔Stripe price swaps.
  if (!sub.stripeSubscriptionId) {
    throw new Error('Please use Stripe checkout or M-Pesa to subscribe to the new plan. Your current plan will remain active until the new subscription activates.');
  }

...
export async function reactivateSubscription(
  ...
  if (hasPaystackSub) {
    // FIX-REACTIVATE-3: Fetch the live subscription state from Paystack BEFORE calling
    // enable. Previously we called paystackEnable blindly, which fails with cryptic
    // errors in two legitimate situations:
    //   (a) The subscription is already "active" on Paystack (paystackDisable failed
    //       silently during cancel but DB was updated — enable returns "already active").
    //   (b) The stored email_token is stale. Paystack may return a fresh token in the
    //       GET response; use it instead of the cached value.
    // By checking status first we can give a precise error for each state and avoid
    // calling enable unnecessarily.
    ...
    // Refresh email_token from Paystack — it is the same value but guards against the
    // edge case where our stored token drifted (e.g. Paystack regenerated it).
    ...
    if (liveSub.status === 'active') {
      // Already active on Paystack (paystackDisable may have failed silently earlier).
      // No API call needed — just reset the DB flags below.
      console.log(`[reactivateSubscription] Sub ${sub.id} is already active on Paystack — skipping enable call`);
    } else if (liveSub.status === 'non-renewing') {
      // Expected state after cancel-at-period-end. Re-enable the subscription.
      await paystackEnable(subscriptionCode!, emailToken!);
    } else {
      // "cancelled", "completed", "attention" — cannot be re-enabled via API.
      throw new Error(
        `Your Paystack subscription is in "${liveSub.status}" state and cannot be reactivated. ` +
        'Please start a new subscription.',
      );
    }
  }

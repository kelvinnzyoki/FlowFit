...
export async function cancelSubscription(
  ...
  if (hasPaystackSub) {
    if (immediately) {
      ...
      // "Cancel at period end" — DO NOT call Paystack's /subscription/disable now.
      // Calling it immediately would:
      //   1. Stop the subscription on Paystack's side right now (user loses access today)
      //   2. Trigger Paystack's subscription.disable webhook → sets status=CANCELLED → lockout
      //
      // Correct flow: set cancelAtPeriodEnd=true in DB only. The expiry cron job checks
      // this flag when currentPeriodEnd is reached and calls paystackDisable at that point.
      // User retains full access until their paid period actually ends.
      await prisma.subscription.update({
        where: { id: sub.id },
        data: {

...
export async function getCurrentSubscription(
  ...
  try {
    ...
    return {
      ...
      // FIX-11: paystackSubscriptionCode is intentionally NOT returned to the client.
      // Paystack's POST /subscription/manage/email/ only requires the subscription code,
      // meaning anyone who intercepts this value can trigger Paystack management emails
      // to the user's inbox (containing card-update and cancellation links) without
      // going through our auth layer. The field is kept backend-only; the frontend uses
      // /subscriptions/billing-portal (server-side) to generate scoped management links.
      isPendingActivation,
    };

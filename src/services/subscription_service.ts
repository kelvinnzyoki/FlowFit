...
export async function reactivateSubscription(
  ...
  // FIX-REACTIVATE-2: Primary query — cancelAtPeriodEnd=true, still within period.
  ...
  // FIX-REACTIVATE-2 (safety net): If the subscription.disable webhook fired before
  // FIX-REACTIVATE-1 was deployed, it may have set status=CANCELLED on a period-end-
  // cancel row. Recover that row as long as currentPeriodEnd is still in the future —
  // the user paid for that period and can still reactivate.
  if (!sub) {
  ...
  if (!sub) {
    // Give a specific, actionable message when the period has already ended.
  ...
  if (hasPaystackSub) {
    try {
      ...
      // FIX-REACTIVATE-3: Paystack returns an error when trying to enable a subscription
      // that is already in "completed" or "cancelled" state (period ended on Paystack's side).
      // Surface a clear, actionable message instead of a raw Paystack API error or generic 500.
      ...
      if (
        ...
        throw new Error(
          'Your Paystack subscription has already ended on Paystack\'s side and cannot be re-enabled. ' +
          'Please start a new subscription.',
        );
      }
      throw err;
  ...
  // FIX-REACTIVATE-2: If the row was incorrectly set to CANCELLED by the old webhook
  // bug, restore it to ACTIVE now that reactivation has succeeded on Paystack.

...
if (!PAYSTACK_SECRET_KEY) {
  throw new Error('PAYSTACK_SECRET_KEY is required for expireSubscriptions. Refusing to run expiry job without Paystack verification.');
}
...
async function fetchPaystackSubscriptionStatus(
  ...
  try {
    ...
    throw new Error(`Paystack status check failed for subscription ${subscriptionCode}: ${err?.message ?? err}`);
  }
}

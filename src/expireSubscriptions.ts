...
// FIX-3: Fail loudly on missing secret key. The previous ?? '' fallback silently
// sent unauthenticated requests that returned null from fetchPaystackSubscriptionStatus,
// causing the 'if (psStatus === "active") continue' guard to never fire and every
// TRIALING subscription to be incorrectly expired on the next cron run.
if (!process.env.PAYSTACK_SECRET_KEY) {
  throw new Error('[expireSubscriptions] PAYSTACK_SECRET_KEY env var is required but not set');
}
...
export async function expireStaleSubscriptions(): Promise<void> {
  ...
  for (const sub of staleIncomplete) {
    // ISSUE-9: Skip Paystack rows that have been paid but whose subscription.create
    // webhook hasn't fired yet. These rows have paystackReference set (payment was
    // taken) and currentPeriodEnd in the future. Paystack retry backoff can exceed
    // 24 hours; expiring a paid row here revokes access for a paying customer.
    if (
  ...
  // ISSUE-5: Step 3 (scheduled downgrades) intentionally removed.
  // applyPendingDowngradeIfDue() in subscription_service.ts handles this lazily
  // inside getCurrentSubscription(). That path creates the new Paystack subscription
  // on the lower plan and properly manages billing.
  // Running a duplicate cron here previously won: it cleared scheduledPlanId without
  // creating a Paystack subscription, so the lazy path found nothing to do and billing
  // stopped entirely for downgraded users.
}

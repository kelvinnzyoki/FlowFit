  ...
  private async buildUserContext(userId: string): Promise<UserContext> {
    ...
    if (enrollment) {
      ...
      program = {
        ...
        nextExercises: (dayData?.exercises ?? []).map(de => de.exercise?.name ?? de.exerciseName ?? '').filter(Boolean),
      };
    }
...
${generated.reply}`,
  ...
  let selectedWeek = enrollment?.program.weeks.find(w => w.weekNumber === ctx.program!.currentWeek) ?? enrollment?.program.weeks[0];
  let selectedDay = selectedWeek?.days.find(d => d.dayNumber === ctx.program!.currentDay) ?? selectedWeek?.days[0];

  if (!selectedDay?.exercises?.length && enrollment?.program.weeks?.length) {
    selectedWeek = enrollment.program.weeks.find(w => w.days?.some(d => d.exercises?.length)) ?? selectedWeek;
    selectedDay = selectedWeek?.days?.find(d => d.exercises?.length) ?? selectedDay;
  }
  ...
  if (!dayEx.length) {
    ...
    return {
      ...generated,
      reply:
        `I found your active program (${ctx.program.name}), but its current day has no exercises attached yet. ` +
        `So I used your enrolled-program context, recent history, goal, level, and metrics instead.\n\n${generated.reply}`,
      data: { ...(generated.data ?? {}), source: 'program_history_goal_fallback' },
    };
  }

  const shownWeek = selectedWeek?.weekNumber ?? ctx.program.currentWeek;
  const shownDay = selectedDay?.dayNumber ?? ctx.program.currentDay;

  const exercisesList = dayEx.map((de, i) => {
    const exName2 = de.exercise?.name     ?? de.exerciseName ?? 'Exercise';
    const exCat2  = (de.exercise?.category ?? 'General');
    const desc    = de.exercise?.description ? ` — ${de.exercise.description}` : '';
    return `${i + 1}. **${exName2}** [${exCat2}]${desc}`;
  }).join('\n');
  ...
  const reply = `**${ctx.program.name}** — Week ${shownWeek}, Day ${shownDay}\n\n` +
                `${exercisesList}\n\n` +
                `Warm up 5–7 minutes • Rest 60–90 seconds between sets${fatNote}\n\n` +
                `${this.workoutFuelPlan(ctx, ctx.program.name + ' Week ' + shownWeek + ' Day ' + shownDay)}\n\n` +
                `Tell me when you're done and I'll log it for you!`;

  return {
    success: true,
    reply,
    data: { exercises: dayEx.map(de => de.exercise).filter(Boolean), week: shownWeek, day: shownDay },
  };
  }
...
${this.workoutFuelPlan(ctx, base.title)}`,
  ...
  if (!selectedWeek?.days?.length && enrollment?.program.weeks?.length) {
    selectedWeek = enrollment.program.weeks.find(w => w.days?.length) ?? selectedWeek;
  }
  ...
  if (!days.length) {
    ...
    return {
      success: true,
      reply: `I found your active program (${ctx.program.name}), but this week's schedule has no saved exercise days. I built a flexible week using your program context, workout history, goal, and level.\n\n` +
        `Week Plan:\n• Day 1: ${base.title}\n• Day 2: Mobility + 25 min walk\n• Day 3: Repeat main lifts with 1 extra rep per set if RPE < 8\n• Day 4: Rest or light stretching\n• Day 5: Full-body conditioning\n\n${this.workoutFuelPlan(ctx, base.title)}`,
      data: { source: 'program_history_goal_fallback', workout: base },
    };
  }
  ...
  private nutritionPlan(ctx: UserContext): CoachResponse {
    ...
    return {
      ...
      reply: `Personalised Nutrition Plan — ${ctx.fitnessGoal.toUpperCase()}\n\n` +
        ...
        mealLines + '\n\n' +
        `Key Rules:\n${tips}`,
      data: { targets: { calories: cal, protein: pro, carbs: carb, fat }, meals: regionalMealOptions(ctx.region).slice(0, 5), predictedWorkout, region: normalizeRegion(ctx.region) },
    };
  }
